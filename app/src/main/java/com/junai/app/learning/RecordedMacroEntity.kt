...
data class RecordedStep(
    val actionType: String,           // "TAP", "LONG_PRESS", "TYPE", "SWIPE", "SLIDE", "HOME", or "RECENTS"
    ...
    // PHASE D (checkbox/switch/radio refinement). Only meaningful for a
    // TAP on a checkable element (isCheckable == true) — the checked
    // state right after the recorded tap. Assumes isChecked() already
    // reflects the post-click state by the time TYPE_VIEW_CLICKED
    // fires, which matches standard Android behavior (the click is
    // processed before the event is dispatched). Lets replay be
    // idempotent: if the element ALREADY matches this state when the
    // macro runs again, tapping it anyway would flip it to the WRONG
    // state — see ActionEngine.tapStep()'s use of this field.
...
data class RecordedMacroEntity(
    ...
    // PHASE G (Health loop v1). Updated once per replay attempt, right
    // when MacroReplayEngine.replay() knows the true outcome — see its
    // recordMacroHealth() for why this is tracked here directly instead
    // of derived from StepOutcomeEntity's per-step log.
