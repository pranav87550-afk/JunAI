...
interface RecordedMacroDao {
    ...
    // PHASE G (Health loop v1) — see MacroReplayEngine.recordMacroHealth().
    @Query("UPDATE recorded_macros SET consecutiveFailures = :consecutiveFailures, lastRunSucceeded = :succeeded WHERE id = :id")
    suspend fun updateHealth(id: Int, succeeded: Boolean, consecutiveFailures: Int)
}
