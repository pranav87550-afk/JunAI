...
class LearningRepository(private val context: Context) {
    ...
    // ==================== AUTO-LEARN FROM RAG ====================
    // Called only from the Qwen3+RAG success path (ChatIntentHandler),
    // and only when the answer was actually RAG-grounded (curated facts
    // backed it, not raw Qwen3 chit-chat) — that's the caller's job to
    // gate, this function trusts it's already been RAG-checked.
    //
    // Deliberately conservative about what it's allowed to touch:
    //   - No existing row for this question  -> insert a fresh one,
    //     tagged with its own category so the Knowledge tab can visually
    //     tell auto-learned entries apart from manually-trained ones.
    //   - Existing row already flagged needsCorrection -> skip. A
    //     thumbs-down (or Pranav manually) already said this answer was
    //     wrong; a fresh auto-save must never silently overwrite that.
    //   - Existing row from a DIFFERENT category (i.e. manually trained,
    //     or trained some other way) -> skip. Auto-learn only ever
    //     refreshes rows it created itself, never someone else's entry.
    //   - Existing row that IS our own earlier auto-learned row -> just
    //     refresh the answer text (facts/RAG content can improve between
    //     sessions), don't insert a duplicate row.
    suspend fun autoLearnFromRag(
        question: String,
        answer: String,
        category: String = "RAG-Auto"
    ) {
        val normalized = question.lowercase().trim()
        if (normalized.isEmpty() || answer.isBlank()) return

        val existing = learningDao.getKnowledgeByQuestion(normalized)

        when {
            existing == null -> trainKnowledge(
                question = normalized,
                answer = answer,
                category = category
            )
            existing.needsCorrection -> {
                // leave flagged rows alone
            }
            existing.category != category -> {
                // leave manually-trained / other-source rows alone
            }
            else -> {
                learningDao.updateKnowledgeAnswerOnly(existing.id, answer)
                refreshStats()
            }
        }
    }
    ...
    // See KnowledgeItem.needsCorrection's doc comment — Negative Responses
    // and Learning Center's Knowledge tab now read/write the same rows.

    /**
     * Called on thumbs-down. If [question] already has a trained KnowledgeItem,
     * flags THAT row as needing correction (keeping its id, so history like
     * timesAsked/timesCorrect isn't lost) instead of creating a duplicate.
     * If no row exists yet (Jun improvised an answer that was never
     * explicitly trained), inserts a fresh one already flagged.
     */
    suspend fun flagNeedsCorrection(question: String, badAnswer: String) {
        val normalized = question.lowercase().trim()
        val existing = learningDao.getKnowledgeByQuestion(normalized)
        if (existing != null) {
            learningDao.flagNeedsCorrection(existing.id, badAnswer)
        } else {
            learningDao.insertKnowledge(
                KnowledgeItem(question = normalized, answer = badAnswer, needsCorrection = true)
            )
        }
        refreshStats()
    }

    suspend fun getItemsNeedingCorrection(): List<KnowledgeItem> = learningDao.getItemsNeedingCorrection()

    /** Saves the corrected answer onto the SAME row and clears needsCorrection. */
    suspend fun resolveCorrection(id: Int, correctAnswer: String) {
        learningDao.resolveCorrection(id, correctAnswer)
        refreshStats()
    }
...
suspend fun trainKnowledgeWithChain(
    ...
    suspend fun findAnswer(query: String): SearchResult {
        ...
        // needsCorrection rows are skipped here on purpose: a row only
        // gets that flag via a thumbs-down on a previously wrong answer
        // (see flagNeedsCorrection), and getKnowledgeByQuestion's query
        // doesn't distinguish it from a trusted one. Without this check
        // a flagged-wrong answer kept being re-served at HIGH confidence
        // on the exact same question, permanently bypassing Qwen3+RAG —
        // the actual cause of "dal kaise banaye" repeating the same
        // wrong cached answer no matter how good the RAG facts were.
        val exactKnowledge = learningDao.getKnowledgeByQuestion(q)
        if (exactKnowledge != null && !exactKnowledge.needsCorrection) {
    ...
    private suspend fun fuzzySearch(query: String): SearchResult? {
        // needsCorrection rows excluded — same reasoning as findAnswer()'s
        // exact-match step: a flagged-wrong answer shouldn't resurface via
        // a near-identical rephrasing either, just because the wording
        // wasn't byte-for-byte identical to what got thumbs-downed.
        val allKnowledge = learningDao.getAllKnowledge().filter { !it.needsCorrection }
