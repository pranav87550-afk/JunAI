 ...
 * Phase 10 (NEW): PlanRepository/GoalDecomposer — "plan: buy milk, clean
 * room" creates a tracked multi-step plan; "next step" / "mark step done"
 * progress it. Unlike the other intercepts, plan queries are deterministic
 * (always produce a response, even "no active plan"), so this one never
 * falls through to proceedWithIntentHandling — it always returns.
 *
...
class ChatIntentHandler(
    ...
    fun handle(intentResult: IntentDetector.IntentResult, target: String, text: String, recyclerView: RecyclerView) {
        ...
        // 1f. Phase 10 (NEW) — plan command intercept. Deterministic, so it
        // always returns (never falls through) — even "no active plan" is
        // a valid, complete response, not a miss to recover from.
