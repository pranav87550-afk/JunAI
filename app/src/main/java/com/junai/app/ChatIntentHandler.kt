 ...
 * Phase 7 (NEW): ReflectionRepository — runs at most once per calendar day
 * (checked in init), comparing today's stats against the last reflection to
 * produce a "what did I learn / where did I fail" log. Answered via a new
 * question intercept ("what did you learn today?", "show reflection").
 *
...
class ChatIntentHandler(
    ...
    fun handle(intentResult: IntentDetector.IntentResult, target: String, text: String, recyclerView: RecyclerView) {
        ...
        // 1e. Phase 7 (NEW) — reflection question intercept (also before dispatch)
