...
class ChatIntentHandler(
    ...
    /**
     * BUGFIX: cancel() alone wasn't enough. On Pranav's weak SM6115 the
     * CPU is already maxed out running Qwen3 inference, so cancellation
     * doesn't reach the native generateStream() loop instantly — a few
     * more onDelta() callbacks fire in that gap, each posting its own
     * "Jun is thinking…" update to the SAME bubble via updateMessageAt(),
     * silently overwriting the "Jun response is interrupted" text right
     * after it was set (confirmed on-device: button flipped back to
     * round, but the thinking animation just kept going with no
     * interrupted message ever visible). This flag is checked before
     * every UI update from the streaming collect loop below, so once
     * interruptGeneration() sets it, no further delta can clobber the
     * interrupted message — regardless of how many extra tokens sneak
     * through before native cancellation actually lands.
     */
     ...
     * suspension point — interruptRequested (checked in the collect loop)
     * is what actually stops a lingering delta from overwriting this.
     */
                ...
                activeJob = scope.launch {
                    ...
                    if (!willNeedFallback) {
                        ...
                        if (ragLookup is RagLookup.Verbatim) {
                            ...
                            try {
                                ...
                                com.junai.app.ml.GGUFChatEngine.streamChat(text, combinedContext, history)?.collect { state ->
                                    ...
                                    // interruptRequested doc comment above.
                                    // Without this check one of these deltas
                                    // would silently overwrite that message
                                    // right back to "Jun is thinking…".
                                    if (interruptRequested) return@collect
                                    finalThinking = state.thinkingSoFar
                                    finalAnswer = state.answerSoFar
                                    withContext(Dispatchers.Main) {
                                        if (interruptRequested) return@withContext
                                        chatAdapter.updateMessageAt(
                                            thinkingMsgIndex,
                                            ChatMessage(
                        ...
                        withContext(Dispatchers.Main) {
                            ...
                            // Same race as the collect guard above — if the
                            // job reached here despite being cancelled
                            // (native cancellation hadn't landed yet), don't
                            // let the finalization logic below overwrite the
                            // "Jun response is interrupted" message with a
