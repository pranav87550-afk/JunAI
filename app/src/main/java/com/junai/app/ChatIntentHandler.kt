 ...
 * Phase 5: KnowledgeGraphRepository — connects facts into chains, answers
 ...
 * Phase 8: ConfidenceEngine/ConfidenceRepository — formalized HIGH/MEDIUM/LOW
 * thresholds, LOW_CONFIDENCE vs NO_MATCH logging distinction.
 * Phase 9 (NEW): CuriosityEngine/CuriosityRepository — when Jun can't answer
 * a genuine question, it asks a follow-up instead of guessing, then learns
 * from whatever the user says next via the existing knowledge base
 * (learningRepo.trainKnowledge). The "is this message the answer to my
 * pending question?" check is the HIGHEST PRIORITY thing in handle() — it
 * runs before confusion detection and before the semantic/graph/reflection
 * intercepts, since literally any non-command reply should resolve it.
 *
 * IMPORTANT (lesson from the Phase 4 bug): ALL question-style intercepts
 * (semantic, graph, reflection) run in handle(), BEFORE the IntentDetector
 * dispatch below — never inside the UNKNOWN branch, since IntentDetector
 * can misclassify these questions into other intents.
 */
class ChatIntentHandler(
    ...
    fun handle(intentResult: IntentDetector.IntentResult, target: String, text: String, recyclerView: RecyclerView) {
        ...
        // This runs before confusion/semantic/graph/reflection checks too,
        // since literally any non-command reply should count as the answer.
        if (curiosityRepo.hasPendingQuestion()) {
            ...
            if (looksLikeRealCommand) {
                // User moved on to something else — drop the pending
                // question and fall through to normal handling below.
    ...
    private fun proceedWithIntentHandling(
        ...
        when (intentResult.intent) {
            ...
            // (semantic + graph + reflection checks no longer live here —
            // they run earlier in handle(), before this dispatch is reached)
                        ...
                        Handler(Looper.getMainLooper()).postDelayed({
                            ...
                            val response: String = when {
                                ...
                                searchResult.answer != null && confidenceLevel == ConfidenceEngine.ConfidenceLevel.MEDIUM ->
                                    ...
                                    // instead of guessing/hallucinating an answer.
