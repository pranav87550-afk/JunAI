...
class ChatIntentHandler(
    ...
    private suspend fun buildRagContext(query: String): String? {
        ...
        if (matches.isEmpty()) {
            android.util.Log.d("RAG", "No knowledge-base match for: \"$query\"")
            // Also written to Breadcrumb (see Breadcrumb.kt) — same trail
            // SplashActivity already shows on next launch, so RAG hits/
            // misses are checkable without adb/Termux, straight from the
            // phone. This was previously logcat-only, which needed a PC
            // or a rooted logcat-reader app to actually see.
            com.junai.app.ml.Breadcrumb.log(activity, "RAG: no match for \"$query\"")
            return null
        }
        // Debug visibility — check via Termux/wireless adb logcat (tag
        // "RAG") to tell apart "RAG retrieved the right fact but Qwen3
        // ignored it" from "RAG never retrieved anything" the next time
        // an answer looks hallucinated despite a matching domain fact
        // existing (e.g. the "how to make dal" case — this log line
        // will show whether cooking_012 was actually retrieved or not).
        android.util.Log.d("RAG", "Matched ${matches.size} for \"$query\": ${matches.joinToString { it.id }}")
        com.junai.app.ml.Breadcrumb.log(
            activity,
            "RAG: matched ${matches.size} for \"$query\": ${matches.joinToString { it.id }}"
        )
        ...
        return "[Known facts — base your answer on these, do not invent extra details beyond them:\n$facts]"
    }
