 ...
 *
 * Phase 4 (NEW): SemanticMemoryRepository is wired in two places:
 *  - Write path: when isPersonalStatement(text) is true, we already extract
 *    a UserPreferenceManager fact; now we ALSO try to extract a structured
 *    semantic fact (predicate/object/category) for richer lookups later.
 *  - Read path: inside the UNKNOWN branch, before falling back to trained
 *    commands / knowledge base, we check if this is a semantic question
 *    ("what language do I like?") and answer directly from stored facts.
 */
class ChatIntentHandler(
    ...
    fun handle(intentResult: IntentDetector.IntentResult, target: String, text: String, recyclerView: RecyclerView) {
        ...
        if (IntentDetector.isPersonalStatement(text)) {
            ...
            // Phase 4: also try to extract a structured semantic fact
            // (predicate/object/category) for richer "what do I like?" lookups.
        ...
        when (intentResult.intent) {
            ...
            // ── UNKNOWN — semantic memory check, then trained command lookup + learning fallback ──
            IntentDetector.Intent.UNKNOWN -> {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    ...
                    // 0. Phase 4 — check semantic memory first
                    // ("what language do I like?" -> resolved from stored facts)
                    val semanticAnswer = semanticMemoryRepo.answerQuery(text)
                    if (semanticAnswer != null) {
                        withContext(Dispatchers.Main) {
                            hideTyping()
                            chatAdapter.addMessage(ChatMessage(semanticAnswer, isUser = false))
                            recyclerView.scrollToPosition(messages.size - 1)
                            onSaveChat()
                            speak(semanticAnswer)
                        }
                        return@launch
                    }
