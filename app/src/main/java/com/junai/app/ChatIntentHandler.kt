...
class ChatIntentHandler(
     ...
     * every token here is taken out of the shared conversation budget
     * (see ChatEngine.MAX_TURNS_BEFORE_RESET), so this shouldn't grow
     * into a large block. Only non-default/meaningful fields are
                    ...
                    if (!willNeedFallback) {
                        ...
                        try {
                            ...
                            val combinedContext = listOfNotNull(ragContext, userContext)
                                .joinToString("\n")
                                .ifBlank { null }
                            com.junai.app.ml.ChatEngine.streamChat(text, combinedContext)?.collect { state ->
