                        ...
                        Handler(Looper.getMainLooper()).postDelayed({
                            ...
                            val response: String = when {
                                ...
                                searchResult.answer != null && confidenceLevel == ConfidenceEngine.ConfidenceLevel.MEDIUM ->
                                    ...
                                    // forUnknown() template — try Qwen3
                                    // (ChatEngine) for a real answer.
                                    // Fire-and-continue init like the
                                    // other ml/ engines, never blocks this
                                    // turn on model load finishing; if not
                                    // ready yet or generation fails,
                                    // qwenResponse is null and behavior
                                    // below is unchanged from before this
                                    // wiring existed.
                                    if (!com.junai.app.ml.ChatEngine.isReady()) {
                                        com.junai.app.ml.ChatEngine.init(activity)
                                    }
                                    val qwenResponse = com.junai.app.ml.ChatEngine.tryChat(text)
                                    when {
