        ...
        when (intentResult.intent) {
            ...
            IntentDetector.Intent.UNKNOWN -> {
                ...
                scope.launch {
                    ...
                    val qwenResponse: com.junai.app.ml.ChatEngine.ChatResponse? = if (willNeedFallback) {
                        ...
                        Handler(Looper.getMainLooper()).postDelayed({
                            ...
                            val response: String = when {
                                ...
                                searchResult.answer != null && confidenceLevel == ConfidenceEngine.ConfidenceLevel.MEDIUM ->
                                    ...
                                    // TODO (thinking UI, separate follow-up
                                    // item): qwenResponse.thinking is
                                    // available here but not yet shown
                                    // anywhere — once the collapsible
                                    // "show thinking" bubble exists, wire
                                    // it in around here rather than
                                    // dropping it.
                                    when {
                                        qwenResponse?.answer?.isNotBlank() == true -> {
                                            respondedViaQwen = true
                                            qwenResponse.answer
                                        }
                            ...
                            if (response.isNotEmpty()) {
                                ...
                                recyclerView.scrollToPosition(messages.size - 1)
                                onSaveChat()
                                // Qwen3 replies are NOT auto-spoken — per
                                // Pranav's instruction, TTS for these only
                                // fires when the user manually taps that
                                // message's speak button (already wired
                                // via ChatAdapter's onSpeak — see
                                // MainActivity.kt line ~195). Every other
                                // response type keeps the existing
                                // auto-speak behavior unchanged.
                                if (!respondedViaQwen) {
                                    speak(response)
                                }
                            }
