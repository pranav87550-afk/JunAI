...
class ChatIntentHandler(
    ...
    fun handle(intentResult: IntentDetector.IntentResult, target: String, text: String, recyclerView: RecyclerView) {
        ...
        if (agentEngine.isTaskRunning() && AgentEngine.looksLikeStopCommand(text)) {
            scope.launch {
                ...
                withContext(Dispatchers.Main) {
                    reply(ack, recyclerView, isSystem = true)
                }
            }
            return
        }
        ...
        if (fuzzyMacroAwaitingConfirm != null) {
            ...
            if (com.junai.app.learning.TriggerMatcher.isAffirmative(text)) {
                if (MacroReplayEngine.isReplaying || agentEngine.isTaskRunning()) {
                    reply("Ek kaam pehle se chal raha hai — pehle wo complete hone do, phir dobara try karo.", recyclerView, isSystem = true)
                    return
                }
                reply("Thik hai, kar rahi hoon... 🎬", recyclerView, isSystem = true)
                scope.launch {
        ...
        if (context.isUserConfused(text)) {
            ...
            reply(hint, recyclerView, isSystem = true)
        ...
        scope.launch {
            ...
            if (macro != null) {
                ...
                withContext(Dispatchers.Main) { reply("Ye maine seekha hai — kar rahi hoon... 🎬", recyclerView, isSystem = true) }
            ...
            if (fuzzy != null) {
                ...
                withContext(Dispatchers.Main) {
                    reply("Ye lagta hai \"${fuzzy.macro.displayPhrase}\" — chalayu?", recyclerView, isSystem = true)
                }
                return@launch
            }
    ...
    private fun proceedWithIntentHandling(
        ...
        when (effectiveIntent) {
            ...
            IntentDetector.Intent.CLEAR_CHAT -> {
                ...
                reply(builder().forIntent(IntentDetector.Intent.CLEAR_CHAT), recyclerView, isSystem = true)
            }
            ...
            IntentDetector.Intent.OPEN_APP -> {
                if (target.isNotEmpty()) {
                    ...
                    "Konsa app open karun? \uD83E\uDD14" else "Which app should I open? \uD83E\uDD14", recyclerView, isSystem = true)
            ...
            IntentDetector.Intent.CALL_CONTACT -> {
                ...
                } else reply(builder().forIntent(IntentDetector.Intent.CALL_CONTACT, target), recyclerView, isSystem = true)
                recyclerView.scrollToPosition(messages.size - 1)
                onSaveChat()
            }
                ...
                activeJob = scope.launch {
                    ...
                    when (passiveOutcome) {
                        is com.junai.app.passive.PassiveExecutionCoordinator.Outcome.Executed -> {
                            withContext(Dispatchers.Main) {
                                hideTyping()
                                reply(passiveOutcome.message, recyclerView, isSystem = true)
                            }
                            return@launch
                        }
                        ...
                        is com.junai.app.passive.PassiveExecutionCoordinator.Outcome.HelpPopupShown -> {
                            ...
                            withContext(Dispatchers.Main) {
                                hideTyping()
                                passiveOutcome.message?.let { reply(it, recyclerView, isSystem = true) }
                            }
                            return@launch
                        }
                    ...
                    if (!willNeedFallback) {
                        ...
                        withContext(Dispatchers.Main) {
                            ...
                            val (response, spokenAloud) = if (finalAnswer.isNotBlank()) {
                                ...
                                chatAdapter.updateMessageAt(
                                    thinkingMsgIndex,
                                    ChatMessage(text = fallback, isUser = false, timestamp = thinkingMsgTimestamp, isSystem = true)
                                )
                                fallback to true
                            }
    ...
    private fun reply(response: String, recyclerView: RecyclerView, isSystem: Boolean = false) {
        _lastResponse = response
        chatAdapter.addMessage(ChatMessage(response, isUser = false, isSystem = isSystem))
        recyclerView.scrollToPosition(messages.size - 1)
        onSaveChat()
        speak(response)
    }
