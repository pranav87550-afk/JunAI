 ...
 *
 * Phase 2: every turn is captured into MemoryRepository, scored via
 * ImportanceEngine, letting the hybrid memory system decide what to keep.
 *
 * Phase 4: SemanticMemoryRepository wired in two places:
 *  - Write path: when isPersonalStatement(text) is true, we ALSO extract a
 *    structured semantic fact (predicate/object/category) for lookups.
 *  - Read path (FIXED): the semantic-question check now happens BEFORE the
 *    IntentDetector dispatch below, not inside the UNKNOWN branch — because
 *    IntentDetector can classify a question like "what do I like?" as
 *    USER_INFO/other (it contains "i like"), so it never reached UNKNOWN.
 *    If the text looks like a semantic question but no fact is found yet,
 *    we fall through to normal intent handling instead of dead-ending.
 */
class ChatIntentHandler(
    ...
    fun handle(intentResult: IntentDetector.IntentResult, target: String, text: String, recyclerView: RecyclerView) {
        ...
        // 1c. FIXED (Phase 4) — semantic question intercept, runs BEFORE
        // IntentDetector's own classification is trusted. If this doesn't
        // look like a semantic question at all, resolve() returns null
        // immediately (cheap, synchronous, no DB hit) and we just continue.
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (answer != null) {
                        ...
                        // Looked like a semantic question but nothing stored yet
                        // for it — fall through to normal intent handling.
    ...
    // ─────────────────────────────────────────────────────────────
    // NORMAL INTENT DISPATCH (unchanged from Phase 2/3, just extracted
    // into its own function so the semantic intercept above can call it
    ...
    private fun proceedWithIntentHandling(
        ...
        if (IntentDetector.isPersonalStatement(text)) {
            ...
            // Also try to extract a structured semantic fact for richer lookups.
            CoroutineScope(Dispatchers.IO).launch {
        ...
        when (intentResult.intent) {
            ...
            // (semantic check no longer lives here — it now runs earlier,
            // in handle(), before this dispatch is even reached)
        ...
        // ImportanceEngine scores it — low-value turns (greetings etc.)
        // fade out automatically via runMaintenance(), nothing to filter here.
        CoroutineScope(Dispatchers.IO).launch {
