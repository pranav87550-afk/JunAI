 ...
 * Phase 6: RuleRepository/ReasoningEngine — proactive IF/AND/OR/NOT
 * recommendations hooked into GREET and TELL_BATTERY.
 * Phase 7: ReflectionRepository — once-per-day "what did I learn" log.
 * Phase 8 (NEW): ConfidenceEngine/ConfidenceRepository — formalizes the
 * 90%/70% knowledge-base thresholds that already existed (previously
 * hardcoded directly in the UNKNOWN branch) into one place, and now
 * distinguishes "found an answer but wasn't confident" (LOW_CONFIDENCE)
 * from "found nothing at all" (NO_MATCH) when logging to FailureLog —
 * which Phase 7's reflection can now tell apart automatically.
 *
        ...
        when (intentResult.intent) {
            ...
            IntentDetector.Intent.UNKNOWN -> {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    ...
                    // instead of separate ">= 90f" / ">= 70f" literals.
                                ...
                                searchResult.answer != null && confidenceLevel == ConfidenceEngine.ConfidenceLevel.MEDIUM ->
                                    ...
                                    CoroutineScope(Dispatchers.IO).launch {
                                        if (searchResult.answer != null) {
                                            // We found something, just weren't confident
                                            // enough — log as LOW_CONFIDENCE so Phase 7's
                                            // reflection can tell this apart from NO_MATCH.
