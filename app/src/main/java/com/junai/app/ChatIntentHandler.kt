 ...
 * Phase 5 (NEW): KnowledgeGraphRepository — connects facts into chains
 * ("user -> likes -> python -> used for -> jun ai") and answers
 * relationship questions ("how are X and Y related?", "what is X used for?",
 * "tell me about X?").
 *
 * IMPORTANT (lesson from the Phase 4 bug): ALL question-style intercepts
 * (semantic AND graph) run in handle(), BEFORE the IntentDetector dispatch
 * below — never inside the UNKNOWN branch, since IntentDetector can
 * misclassify these questions into other intents.
 */
class ChatIntentHandler(
    ...
    fun handle(intentResult: IntentDetector.IntentResult, target: String, text: String, recyclerView: RecyclerView) {
        ...
        // 1d. Phase 5 (NEW) — graph question intercept (also before dispatch,
        // same reasoning as 1c: IntentDetector could misclassify these too)
    ...
    private fun proceedWithIntentHandling(
        ...
        // Phase 5 — try to extract a graph relation from EVERY message, not
        // just personal statements (covers third-person facts like
        // "Python is used for Jun AI" too). GraphRelationExtractor itself
        // returns null harmlessly when nothing matches, so this is cheap.
        CoroutineScope(Dispatchers.IO).launch {
        ...
        when (intentResult.intent) {
            ...
            // (semantic + graph checks no longer live here — they run
            // earlier in handle(), before this dispatch is even reached)
