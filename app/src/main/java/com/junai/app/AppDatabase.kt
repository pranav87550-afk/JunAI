...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        // NEW — Passive Learning Phase 4: track consecutive failures per
        // edge for the "3 in a row triggers steeper drop + sikhao
        // suggestion" rule.
        private val MIGRATION_17_18 = object : Migration(17, 18) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE passive_edges ADD COLUMN consecutiveFailures INTEGER NOT NULL DEFAULT 0")
            }
        }

        // PHASE G — Record & Replay Health Loop v1: same consecutive-
        // failure tracking pattern as MIGRATION_17_18 above, applied to
        // recorded_macros (explicitly taught/replayed macros) instead of
        // passive_edges (passively learned paths) — these are two
        // separate tables/systems, so the same column needs adding twice.
        private val MIGRATION_18_19 = object : Migration(18, 19) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE recorded_macros ADD COLUMN consecutiveFailures INTEGER NOT NULL DEFAULT 0")
                database.execSQL("ALTER TABLE recorded_macros ADD COLUMN lastRunSucceeded INTEGER DEFAULT NULL")
            }
        }
