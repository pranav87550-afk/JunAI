...
class VoiceCommandsActivity : AppCompatActivity() {
    ...
    private fun getD(id: Int): Drawable? = ContextCompat.getDrawable(this, id)

    private fun setupManual() {
        val container = findViewById<LinearLayout>(R.id.manualContainer)

        addHeader(container, "🤖 JunAI", "Your Personal AI Companion for Android", "Version 1.0  •  Developer: Pranav  •  Built in Kotlin")

        addSection(container, "🌟", "What is Jun AI?", "#FF1744")
        addCard(container, "Jun AI is a personal AI assistant built for Android. It understands your questions, learns from your inputs, executes commands, and gets smarter over time. Jun works offline for stored knowledge and online for web search and translation.", "#FF1744")

        addSection(container, "✨", "Core Features", "#FF6D00")
        addFeatureCard(container, "💬", "AI Chat", "#FF6D00", "Chat with Jun in natural language. Ask questions, get answers, learn new things. Jun remembers what you teach her and improves with every conversation.")
        addFeatureCard(container, "🎤", "Voice Input (STT)", "#FF6D00", "Tap the mic button to speak instead of type. Jun converts your speech to text and automatically sends it — no need to press send.")
        addFeatureCard(container, "🔊", "Voice Output (TTS)", "#FF6D00", "Jun can speak her responses aloud. Enable Voice in Settings to hear Jun's answers in real time.")
        addFeatureCard(container, "🧠", "Knowledge Memory", "#FF6D00", "Jun stores everything you teach her in a local database. Stored knowledge works completely offline.")
        addFeatureCard(container, "🎓", "Learning Center", "#FF6D00", "Train Jun using 4 tabs: Pending (unanswered questions), Knowledge (stored facts), Commands (custom triggers), Analytics (performance stats).")
        addFeatureCard(container, "📝", "Notes", "#FF6D00", "Create, view, and delete personal notes. All notes are saved locally on your device.")
        addFeatureCard(container, "✅", "To-Do Lists", "#FF6D00", "Add tasks, mark them done, and clear all with confirmation. Jun tracks your pending task count.")
        addFeatureCard(container, "⏰", "Reminder & Alarm", "#FF6D00", "Set reminders with a title and time. Alarms fire even after phone restart. Includes lock screen notification support.")
        addFeatureCard(container, "🌐", "Translator", "#FF6D00", "Translate text between 15 languages using Google Translate. Results are cached locally for instant repeat translations.")
        addFeatureCard(container, "🎵", "Jun DJ (Music Player)", "#FF6D00", "Play songs from your device. Features: song list A-Z, favorites playlist, add to playlist, set ringtone/alarm tone, rename, delete, and next/previous controls.")
        addFeatureCard(container, "🎨", "Sketch Pad", "#FF6D00", "Draw on a canvas with color options, brush size, undo/redo, eraser. Auto-save toggle — drawings save to Pictures/JunAI gallery.")
        addFeatureCard(container, "🔢", "Calculator", "#FF6D00", "Full-featured calculator for arithmetic operations.")
        addFeatureCard(container, "🤖", "Mini-Jun (Floating Bot)", "#FF6D00", "A floating AI assistant overlay that stays on top of all apps. Customize its appearance and behavior in Mini-Jun Settings.")
        addFeatureCard(container, "🛡️", "Permission Centre", "#FF6D00", "View and manage all permissions Jun AI needs. Toggle permissions on/off with explanations for each one.")
        addFeatureCard(container, "📊", "Data Management", "#FF6D00", "Import knowledge in bulk via JSON. Export and manage your stored data.")
        ...
        addCommandCard(container, "Pending Tab", "Shows all questions Jun could not answer. Train each one as Knowledge or Command.", "#4CAF50")
        addCommandCard(container, "Knowledge Tab", "View all stored knowledge. Delete individual entries or clear all.", "#4CAF50")
        addCommandCard(container, "Commands Tab", "View all trained custom commands with trigger phrase and response.", "#4CAF50")
        addCommandCard(container, "Analytics Tab", "View Jun's performance stats — success rate, failure log, and achievements.", "#4CAF50")

        addSection(container, "📚", "How to Train Jun?", "#1565C0")
        addCommandCard(container, "Method 1 — Direct Chat", "Type in Q=A format in chat.\nExample: Eiffel Tower height=330 meters", "#1565C0")
        addCommandCard(container, "Method 2 — Learning Center", "Go to Learning Center → Pending Tab. Train unanswered questions as Knowledge or Commands.", "#1565C0")
        ...
        addCommandCard(container, "Voice Pitch & Speed", "Adjust how Jun sounds using pitch and speed sliders.", "#FF9800")
        ...
        addCommandCard(container, "Microphone", "Required for voice input (STT).", "#1565C0")
        ...
        addCommandCard(container, "Music Favorites", "Tap ⋮ on any song → Add to Favorites.", "#FF6D00")
    ...
    private fun addFeatureCard(parent: LinearLayout, emoji: String, title: String, accentColor: String, description: String) {
        val card = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            background = getD(R.drawable.crystal_song_item)
