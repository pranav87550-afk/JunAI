...
class FloatingBotView(context: Context) : View(context) {
    ...
    // ─── Visor area (fraction of view size) ───────────────────
    // Tuned to Jun's PNG visor position
    private val visorLeftF   = 0.22f
    private val visorTopF    = 0.28f
    private val visorRightF  = 0.78f
    private val visorBottomF = 0.64f

    private val visorRect = RectF()

    // ─── Eye state ────────────────────────────────────────────
    private var blinkProgress = 0f   // 0 = open, 1 = fully closed
    private var touchX = -1f
    private var touchY = -1f

    // ─── Mouth state ──────────────────────────────────────────
    var mouthOpenAmount = 0f         // 0 = closed, 1 = fully open (TTS hook)
    ...
    // Bobbing (managed by FloatingBotService, exposed as offset)
    ...
    private fun loadBotImage() {
        try {
            ...
            botBitmap = BitmapFactory.decodeResource(resources, R.drawable.jun_bot, opts)
        } catch (e: Exception) {
            botBitmap = null
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // Recalculate visor rect every draw (safe)
        ...
        // 3. Visor glow
        val vr = visorRect
        canvas.drawRoundRect(vr, 60f, 60f, visorGlowPaint)

        // 4. Eyes + mouth inside visor
        drawFace(canvas)
    }
    ...
    private fun drawNeonGlow(canvas: Canvas, w: Float, h: Float) {
        ...
        canvas.drawOval(oval, neonBodyGlowPaint)
    }
    ...
    private fun drawFace(canvas: Canvas) {
        ...
        // Eye sizing relative to visor
        val eyeR = vw * 0.13f          // eyeball radius
        val eyeSpacing = vw * 0.22f    // distance from center to each eye
        val eyeY = vcy - vh * 0.08f   // slightly above center
        ...
        drawMouth(canvas, vcx, vcy + vh * 0.22f, vw * 0.28f)
    }
    ...
    private fun drawEye(canvas: Canvas, cx: Float, cy: Float, radius: Float) {
        ...
        // Pupil tracking
        val pupilR = radius * 0.48f
        val maxOffset = radius * 0.38f
        val (px, py) = getPupilOffset(cx, cy, maxOffset)
        canvas.drawCircle(cx + px, cy + py, pupilR, pupilPaint)

        // Glint
        val glintR = pupilR * 0.28f
        canvas.drawCircle(cx + px - pupilR * 0.22f, cy + py - pupilR * 0.22f, glintR, pupilGlintPaint)
    ...
    private fun getPupilOffset(eyeCx: Float, eyeCy: Float, maxOffset: Float): Pair<Float, Float> {
        ...
        // Convert touch to view-local coords (view is positioned by service)
    ...
    private fun drawMouth(canvas: Canvas, cx: Float, cy: Float, halfWidth: Float) {
        when (expression) {
            BotExpression.HAPPY -> drawSmile(canvas, cx, cy, halfWidth, curvature = 0.5f)
            ...
            else -> drawNeutralMouth(canvas, cx, cy, halfWidth)
        }
    }

    private fun drawNeutralMouth(canvas: Canvas, cx: Float, cy: Float, hw: Float) {
        val path = Path()
        path.moveTo(cx - hw, cy)
        path.quadTo(cx, cy + hw * 0.18f, cx + hw, cy)
        canvas.drawPath(path, mouthPaint)
    }

    private fun drawSmile(canvas: Canvas, cx: Float, cy: Float, hw: Float, curvature: Float) {
        val path = Path()
        path.moveTo(cx - hw, cy)
        path.quadTo(cx, cy + hw * curvature, cx + hw, cy)
        canvas.drawPath(path, mouthPaint)
    }

    private fun drawSleepMouth(canvas: Canvas, cx: Float, cy: Float, hw: Float) {
        canvas.drawLine(cx - hw * 0.5f, cy, cx + hw * 0.5f, cy, mouthPaint)
    }

    private fun drawThinkMouth(canvas: Canvas, cx: Float, cy: Float, hw: Float) {
        val path = Path()
        path.moveTo(cx - hw * 0.4f, cy + hw * 0.1f)
        path.quadTo(cx, cy - hw * 0.1f, cx + hw * 0.4f, cy + hw * 0.05f)
        canvas.drawPath(path, mouthPaint)
    }

    private fun drawSpeakMouth(canvas: Canvas, cx: Float, cy: Float, hw: Float) {
        val openY = cy + hw * 0.35f * mouthOpenAmount
        // Fill
        val fillPath = Path()
        fillPath.moveTo(cx - hw * 0.6f, cy)
        fillPath.quadTo(cx, cy - hw * 0.1f, cx + hw * 0.6f, cy)
        fillPath.quadTo(cx, openY + hw * 0.1f, cx - hw * 0.6f, cy)
        fillPath.close()
        canvas.drawPath(fillPath, mouthFillPaint)
        // Outline
        val outPath = Path()
        outPath.moveTo(cx - hw * 0.6f, cy)
        outPath.quadTo(cx, openY, cx + hw * 0.6f, cy)
        canvas.drawPath(outPath, mouthPaint)
    }

    // ──────────────────────────────────────────────────────────
    // TOUCH — pupils follow finger
    // ──────────────────────────────────────────────────────────
