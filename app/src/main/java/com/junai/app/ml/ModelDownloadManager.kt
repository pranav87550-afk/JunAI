...
object ModelDownloadManager {
     ...
     *
     * BUGFIX: originally compared file.length() against a percentage of
     * ModelCatalog's approxSizeBytes — but those are rough hand-guessed
     * numbers (e.g. "~300 MB"), not the real file size, so a fully
     * downloaded model could fail this check forever if the guess was
     * too high (exactly what happened with EmbeddingGemma: guessed
     * ~300MB, real file is ~174MB, so a complete download could never
     * reach 95% of the wrong number). Plain existence is actually the
     * correct check here — ModelDownloadWorker only renames the ".part"
     * file to this final filename once the full stream has completed
     * without error, so if this file exists under its final name at
     * all, the download behind it was already complete.
     */
