...
""".trimIndent()
    ...
    // Same race-condition fix as ChatEngine — MainActivity's warm-up
    // call and the existing per-message lazy-init path can both call
    // init() around the same time; this serializes them instead of
    // letting both construct a native LlmInference concurrently.
    private val initMutex = Mutex()

    suspend fun init(context: Context) {
        if (llmInference != null || initFailed) return
        initMutex.withLock {
        if (llmInference != null || initFailed) return@withLock
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val options = LlmInference.LlmInferenceOptions.builder()
                    ...
                    // BUGFIX: was 256. Per MediaPipe's own docs,
                    // setMaxTokens() is the TOTAL input+output budget,
                    // not just output — our SYSTEM_PROMPT +
                    // FUNCTION_DECLARATIONS alone (sent on every single
                    // call, see buildPrompt()) is already 150-200+
                    // tokens before the user's message or any response
                    // is even counted. 256 left little to no headroom,
                    // which lines up with a native crash happening on
                    // literally every generateResponse() call
                    // regardless of what the user typed — every
                    // official Google example uses 1000-1200 here.
                    .setMaxTokens(1024)
                    .build()
