 ...
 * that existing path.
 *
 * PROMPT FORMAT: FunctionGemma expects a specific developer/user/model
 * turn structure with JSON function declarations in the developer turn
 * (confirmed against a working reference integration of this exact
 * model — see buildPrompt() below). It is NOT the same chat template
 * MediaPipe's generateResponse() would auto-apply for a generic Gemma
 * chat model — the full prompt is built manually here on purpose.
 *
...
object FunctionCallEngine {
    ...
    private const val MODEL_ASSET_PATH = "functiongemma-270M-it.task"
