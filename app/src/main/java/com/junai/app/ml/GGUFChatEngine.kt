    ...
    suspend fun tryChat(text: String): String? {
        ...
        return inferenceMutex.withLock {
            withContext(Dispatchers.Default) {
                ...
                try {
                    LlamaBridge.sessionReset()
                    val finetuneType = LlamaBridge.getModelFinetuneType()
                    val template = LlamaBridge.applyChatTemplate(
                        messages = listOf("user" to text),
                        addAssistantPrefix = true
                    )
                    val prompt = template ?: text
                    val diag = "[finetune=${finetuneType ?: "null"}, template=${if (template != null) "FOUND" else "MISSING (raw fallback used)"}]\n\n"
                    ...
                    if (success && sb.isNotBlank()) diag + sb.toString().trim() else diag + "(empty response)"
                } catch (e: Exception) {
