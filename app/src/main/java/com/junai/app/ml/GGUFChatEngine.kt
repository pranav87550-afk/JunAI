    ...
    fun streamChat(text: String): Flow<StreamState>? {
        ...
        return channelFlow {
            inferenceMutex.withLock {
                ...
                // Each call here is treated as an independent, single-shot generation
                // (no conversation history is passed in). Llamatik keeps the KV cache
                // in memory across generateStream() calls, so without resetting it,
                // a second call on top of a first one can overflow CONTEXT_LENGTH and
                // crash natively (no onError, no exception — the process just dies).
                // See: https://github.com/ferranpons/Llamatik sessionReset() docs.
    ...
    suspend fun tryChat(text: String): String? {
        ...
        return inferenceMutex.withLock {
            withContext(Dispatchers.Default) {
                ...
                try {
                    // Same reasoning as streamChat(): clear KV cache before every
                    // independent generation to avoid the native context-overflow crash.
                    LlamaBridge.sessionReset()
