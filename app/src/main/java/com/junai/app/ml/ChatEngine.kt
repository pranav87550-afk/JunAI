...
object ChatEngine {
    ...
    // BUGFIX: with the addition of MainActivity's app-open warm-up call,
    // there are now TWO code paths that can call init() around the same
    // time — the warm-up itself, and the existing per-message lazy path
    // in ChatIntentHandler.kt (which still checks isReady() first, as a
    // safety net in case warm-up hasn't finished yet). Without a lock,
    // both can pass the isReady()==false check before either one
    // finishes, and BOTH proceed to construct+initialize() a full native
    // Engine concurrently — confirmed via breadcrumb trail (two separate
    // "about to construct Engine(engineConfig) [GPU]" lines). That's not
    // just wasted work: two GPU/native inits running at once compete for
    // the same CPU/GPU resources, which was making an ~85s generation
    // take even longer, and made FunctionGemma's classification (running
    // concurrently on the same device) take ~18s for what should be a
    // sub-2s call. This mutex makes a second concurrent init() call wait
    // for the first to finish instead of racing it.
    ...
    suspend fun init(context: Context) {
        ...
        initMutex.withLock {
            ...
            // (warm-up vs per-message path) may have already finished
            // initializing while this call was waiting for the lock.
            if (isReady() || shouldSkipInit()) return@withLock
