...
class WebSearchHelper(
    ...
    private fun fallbackToQwen3OrGiveUp(
        ...
        scope.launch {
            ...
            activity.runOnUiThread {
                hideTyping(typingIndicator, dot1, dot2, dot3)
                val qwenAnswer = response?.answer?.takeIf { it.isNotBlank() }
                val answer = qwenAnswer ?: "I couldn't find an answer for \"$query\". Try rephrasing!"
                chatAdapter.addMessage(ChatMessage(answer, isUser = false, isSystem = qwenAnswer == null))
                recyclerView.scrollToPosition(messages.size - 1)
                onSaveChat()
            ...
            override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {
                ...
                activity.runOnUiThread {
                    try {
                        ...
                        hideTyping(typingIndicator, dot1, dot2, dot3)
                        chatAdapter.addMessage(ChatMessage("Search failed. Try again!", isUser = false, isSystem = true))
                        recyclerView.scrollToPosition(messages.size - 1)
                        onSaveChat()
                    }
                }
