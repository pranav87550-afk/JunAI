...
object IntentDetector {
    ...
    private val researchPhrases = listOf(
        ...
        // NOTE: "kya hai" was here briefly (added to catch patterns like
        // "weather pata karo aur batao") but had to be removed — it's
        // just "what is" in Hindi, so it matched almost ANY Hindi/
        // Hinglish question ("Jun kya hai" among them) and swept it into
        // web-research instead of the router/RAG/Qwen3 pipeline, which
        // is a much worse false-positive than the gap it was trying to
        // fix. The remaining phrases below are specific enough (multi-
        // word, verb-containing) to not have this problem.
        "pata karo", "pata lagao", "ke baare mein batao"
    )
    ...
    private val intentPhrases = mapOf(
        ...
        Intent.USER_INFO to listOf(
            ...
            // BUGFIX 2: also removed "i love" and "i like" — same
            // failure mode ("I love you" was matching "i love" and
            // getting the canned "yaad kar liya maine!" ack instead of
            // ever reaching the chat model). These are emotional/
            // opinion expressions, not facts worth memorizing via this
            // mechanism — better to let them flow through to normal
            // chat, which can respond naturally.
            "my name is", "call me", "mera naam",
            "i live in", "i'm from", "i work",
            "my job", "my hobby", "meri umar",
    ...
    // Used by the SEARCH_WEB pronoun guard below — a bare pronoun left over
    // after stripping a phrase like "tell me about" can't be searched for
    // literally (see BUGFIX note at the resolvedIntent computation).
    private val pronounStopWords = setOf(
        "it", "that", "this", "them", "him", "her", "these", "those",
        "iske", "uske", "ise", "use", "usse", "isse", "vo", "wo", "woh",
        "yeh", "ye", "us", "unke", "unko", "iska", "uska", "isko"
    )
    ...
    fun detect(input: String): IntentResult {
        ...
        // 5.6. BUGFIX: SEARCH_WEB + a pronoun-only leftover entity (e.g. "tell
        // me about it" -> phrase "tell me about" matches, entity extracts to
        // just "it") used to fire a literal web search for "it"/"that"/etc,
        // which has no relation to whatever the user actually meant — it
        // just hits whatever Wikipedia article happens to match that word
        // (e.g. "it" -> the .it Italy domain page). A bare pronoun can only
        // be resolved using conversation history, which IntentDetector
        // doesn't have — so downgrade to UNKNOWN here and let it fall
        // through to Qwen3 (which DOES get history) instead.
        val bareEntity = bestEntity.lowercase().trim()
        val isPronounOnlyEntity = bareEntity.isNotEmpty() &&
            bareEntity.split(Regex("\\s+")).all { it in pronounStopWords }

        // 6. Fall back to UNKNOWN if confidence too low, or if SEARCH_WEB
        // resolved to nothing but a bare pronoun.
        val resolvedIntent = when {
            finalScore < 30 -> Intent.UNKNOWN
            bestIntent == Intent.SEARCH_WEB && (bestEntity.isBlank() || isPronounOnlyEntity) -> Intent.UNKNOWN
            else -> bestIntent
        }
    ...
    private fun scoreMatch(input: String, phrase: String): Int {
        ...
        return when {
            ...
            // Same class of bug as the earlier "i love"/"i am" fix (see
            // wordOverlapScore-input guard right below), but on the
            // PHRASE side instead of the input side: a short 2-word
            // phrase like "what is my battery level" or "what is the
            // current time" only needs ONE shared word ("what") to hit
            // 40-50% overlap against almost any "what is X" question —
            // including ones that have nothing to do with battery or
            // time, like "what is machine learning" (real bug: this
            // fuzzy-matched TELL_BATTERY and fired a battery
            // recommendation alongside the real answer). Mirroring the
            // input-side guard fixes the whole class at once instead of
            // trimming individual phrases one bug report at a time.
            p.split(Regex("\\s+")).size < 3 -> 0
    ...
    fun isPersonalStatement(input: String): Boolean {
        val lower = input.lowercase()
        // BUGFIX: removed "i am", "i'm", "mujhe", "main hun" — same
        // over-generic-phrase problem as the USER_INFO entry in
        // intentPhrases (see that fix's comment), except this is a
        // completely separate check that runs unconditionally on every
        // message regardless of confidence gating, so it needed the
        // same fix applied here too.
        // BUGFIX 2: also removed "i like"/"i love" here too — "I love
        // you" was matching and getting treated as a fact to memorize
        // instead of reaching the chat model. See intentPhrases fix.
        val personalMarkers = listOf(
            "my name", "i live", "i work",
            "my job", "my hobby", "my age", "i'm from", "call me",
            "mera naam", "meri umar", "main rehta"
        )
