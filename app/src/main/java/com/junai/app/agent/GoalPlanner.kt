...
object GoalPlanner {
    ...
    private fun multiStepPlan(params: IntentDetector.AgentTaskParams): List<AgentStep> {
        ...
        for (clause in clauses) {
            // BUGFIX: connector words like "aur"/"phir" only split on
            // explicit connectors — but "WhatsApp khol ke Mom ko message
            // bhejo 'text'" has no such connector between "open" and
            // "send", so it stayed one clause and got only ONE StepType
            // (whichever matched first in inferStepType). That meant
            // either the open-app step never happened, or the send step
            // never happened. Detect this combined open+send pattern and
            // explicitly split it into two real steps instead.
            val openMatch = OPEN_WORD_REGEX.find(clause)
            val sendMatch = SEND_WORD_REGEX.find(clause)
            if (openMatch != null && sendMatch != null && sendMatch.range.first > openMatch.range.first) {
                // BUGFIX: splitting at the SEND word (e.g. "bhejo") cut the
                // clause as "WhatsApp khol ke Papa ko message" | "bhejo '...'"
                // — the contact name ("Papa") ended up trapped in the OPEN
                // part and was never available to CONTACT_BEFORE_KO_REGEX in
                // AgentEngine, which reads it from the TYPE step's
                // description. That silently dropped the contact lookup
                // entirely (contact == null), so Jun never tapped into the
                // right chat and just searched for a message field on the
                // WhatsApp chat-list screen, where none exists.
                // Fix: split right before the "<contact> ko ..." phrase
                // instead, so the contact name travels with the TYPE step.
                val contactKoMatch = CONTACT_KO_START_REGEX.find(clause)
                val splitIndex = contactKoMatch?.range?.first ?: sendMatch.range.first
                val appPart = clause.substring(0, splitIndex).trim()
                val messagePart = clause.substring(splitIndex).trim()
                steps.add(AgentStep(n++, StepType.OPEN, appPart, appPart,
                    "If the app isn't installed or won't open, tell the user"))
                steps.add(AgentStep(n++, StepType.TYPE, messagePart, messagePart,
                    "Message content is always shown before sending — never sent silently"))
                continue
            }
    ...
    private val OPEN_WORD_REGEX = Regex("(?i)\\b(open|khol|kholo|kholna|khola)\\b")
    private val SEND_WORD_REGEX = Regex("(?i)\\b(send|bhejo|bhej)\\b")
    // Finds where the "<contact> ko message/send/bhejo/bhej" phrase begins,
    // so the OPEN/TYPE split point can land BEFORE the contact name instead
    // of after it. Single word only — a 2-word attempt here would greedily
    // swallow the preceding Hindi connector "ke" (as in "khol ke Papa ko
    // message...") as if it were part of the name. Multi-word contact names
    // ("Ravi bhai") aren't handled by this split yet.
    private val CONTACT_KO_START_REGEX =
        Regex("(?i)\\S+\\s+ko\\s+(?:message|msg|send|bhejo|bhej)\\b")

    private fun splitIntoClauses(text: String): List<String> {
        // BUGFIX: connector words were being replaced everywhere in the raw
        // text, INCLUDING inside quoted message content — so a message like
        // "message papa 'mein phir aa rha hai'" got split right through the
        // quotes on the word "phir", leaving the TYPE step with a broken,
        // unterminated quote ("message papa 'mein") and no way to find the
        // actual message text. Fix: pull quoted spans out first, split on
        // connectors, then put the quotes back into whichever clause they
        // landed in — connector words inside quotes are never touched.
        ...
        for (connector in connectorWords) {
            protectedText = protectedText.replace(Regex("(?i)\\b${Regex.escape(connector)}\\b"), "||")
        }

        return protectedText.split("||")
            .map { clause ->
                var restored = clause
                quotes.forEachIndexed { i, q -> restored = restored.replace("\u0000Q$i\u0000", q) }
                restored.trim()
            }
    ...
    private fun inferStepType(clause: String): Pair<StepType, String?> {
        ...
        return when {
            // BUGFIX: "kholo"/"kholna" alone missed extremely common forms
            // like "khol" ("WhatsApp khol ke...") and "khol do"/"khola" —
            // these silently fell through to TYPE/SYSTEM_ACTION instead,
            // skipping the open-app step entirely in multi-step plans.
            containsAny(lower, "open", "khol", "kholo", "kholna", "khola") ->
            ...
            // BUGFIX: "bhejo" alone missed "bhej"/"bhej do"/"bheja" forms,
            // and clauses using only "message"/"msg" with no "send"/"bhejo"
            // word at all (e.g. "message mummy" from "open whatsapp and
            // message mummy") fell through every branch to the default
            // OPEN case, making AgentEngine try to open a nonexistent app
            // literally named "message mummy".
            containsAny(lower, "send", "bhejo", "bhej", "message", "msg") ->
