...
object MacroReplayEngine {
    ...
    suspend fun replay(context: Context, macro: RecordedMacroEntity): String {
        ...
        for ((index, step) in steps.withIndex()) {
            ...
            // A TAP can still fall back to recorded bounds (see tapStep), so
            // only bail out here if there's truly nothing to go on at all —
            // TYPE steps still need a real identifier since bounds alone
            // can't tell us WHICH field to type into reliably.
            ...
            if (target.isNullOrBlank() && !(step.actionType == "TAP" && hasBounds)) {
            ...
            val result = when (step.actionType) {
                ...
                "TYPE" -> {
                    ...
                    val typeTarget = target
                    if (typeTarget.isNullOrBlank()) {
                        ActionResult(false, "is TYPE step ka koi identifier hi nahi mila")
                    } else if (typed.isNullOrBlank()) {
                        ActionResult(false, "record kiya gaya text khaali hai")
                    } else {
                        ActionEngine.typeText(typeTarget, typed)
                    }
                }
