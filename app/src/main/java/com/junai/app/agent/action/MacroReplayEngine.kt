...
object MacroReplayEngine {
     ...
     * whole when-block.
     */
    private suspend fun executeStep(step: RecordedStep): ActionResult {
    ...
    private suspend fun waitForStepEvidence(step: RecordedStep, timeoutMs: Long): Boolean {
        ...
        while (System.currentTimeMillis() < deadline) {
            val node = svc.findBestMatchingNode(
     ...
     */
    private fun detectDrift(expectedPkg: String?): String? {
        if (ScreenContextEngine.looksLikeLockScreen()) {
            return "screen par ek lock/authentication prompt dikh raha hai"
        }
        if (!expectedPkg.isNullOrBlank() && !ScreenContextEngine.isAppInForeground(expectedPkg)) {
