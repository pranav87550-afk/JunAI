...
object MacroReplayEngine {
    ...
    private suspend fun navigateToApp(context: Context, pkg: String): ActionResult {
        ...
        return when {
            ...
            // BUGFIX (root cause of home-screen swipe/tap steps intermittently
            // failing right after this branch reported success — confirmed
            // on-device via log: the very next check saw currentApp still
            // showing the previous app, and separately ColorOS's own
            // "isHomeButtonDisabled" check adds extra delay around the HOME
            // key on this OEM): performGlobalAction(GLOBAL_ACTION_HOME)'s
            // return value only means the request was DISPATCHED, not that
            // the transition finished — unlike expandQuickSettings() below,
            // which already polls isQuickSettingsWindowOpen() before
            // reporting success. This mirrors that same poll-and-verify
            // pattern instead of trusting the dispatch result alone.
            pkg == RecordingEngine.HOME_SCREEN_SENTINEL || pkg == launcherPkg -> {
                if (ScreenContextEngine.isAppInForeground(context, RecordingEngine.HOME_SCREEN_SENTINEL)) {
                    ActionResult(true, "already home")
                } else {
                    val dispatched = JunAccessibilityService.instance?.pressHome() ?: false
                    if (!dispatched) {
                        ActionResult(false, "couldn't go to the home screen — is the accessibility service connected?")
                    } else {
                        val deadline = System.currentTimeMillis() + 1500L
                        var confirmed = false
                        while (System.currentTimeMillis() < deadline) {
                            if (ScreenContextEngine.isAppInForeground(context, RecordingEngine.HOME_SCREEN_SENTINEL)) { confirmed = true; break }
                            delay(100L)
                        }
                        if (confirmed) ActionResult(true, "went home")
                        else ActionResult(false, "home button dabaya, lekin home screen confirm nahi hui — shayad OEM ne delay/block kiya")
                    }
                }
            }
    ...
    private suspend fun replayInternal(context: Context, macro: RecordedMacroEntity, onStepResult: (suspend (Int, Boolean) -> Unit)? = null): String {
        ...
        while (index < steps.size) {
            ...
            // ScreenContextEngine.isAppInForeground() reads currentApp,
            // which is only ever updated from an accessibility event's
            // packageName (see JunAccessibilityService.onAccessibilityEvent).
            // com.android.systemui's Quick Settings shade doesn't reliably
            // fire those events on this OEM (same root cause as the
            // recording-side tap/swipe misses), so currentApp can sit
            // stale — most dangerously, stuck on "systemui" from a
            // previous QS step even after the panel actually closed. That
            // made this check wrongly report "already there", skip
            // navigateToApp()/expandQuickSettings() entirely, and the next
            // step then found no panel to tap. For the systemui step
            // specifically, ask the live window list instead — same
            // instance either way, no event dependency.
            val pkg = step.packageName
            val alreadyThere = if (pkg == "com.android.systemui") {
                JunAccessibilityService.instance?.isQuickSettingsWindowOpen() == true
            } else {
                ScreenContextEngine.isAppInForeground(context, pkg ?: "")
            }
            if (!pkg.isNullOrBlank() && !alreadyThere) {
            ...
            //
            // BUGFIX: a SWIPE step captured via RecordingEngine.
            // captureScrollFromEventFallback (QS panel page-swipe with no
            // source node — see that function's doc comment) intentionally
            // has no resourceId/text/description AND no bounds — its only
            // recorded info is the direction (scrollForward/scrollHorizontal),
            // which ActionEngine.scrollStep()'s generic-screen-swipe
            // fallback uses instead of a position. That's a legitimate,
            // replayable step, not a corrupt one — this gate used to treat
            // it as fatal and abort the whole macro before scrollStep ever
            // got a chance to run its fallback.
            ...
            val isDirectionOnlySwipe = step.actionType == "SWIPE" && step.scrollForward != null
            if (target.isNullOrBlank() && !hasBounds && !isDirectionOnlySwipe) {
