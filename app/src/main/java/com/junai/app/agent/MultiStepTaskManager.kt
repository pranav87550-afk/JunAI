...
// NOTE: AgentTaskEntity and AgentTaskDao used to live in this file. They
// now live in their own files (AgentTaskEntity.kt, AgentTaskDao.kt) in this
// same package, because AppDatabase.kt imports them directly and Room's
// annotation processor needs each @Entity/@Dao in its own compilation unit
// to generate correctly. Nothing else below changed.
...
class MultiStepTaskManager(context: Context) {
    ...
    // BUGFIX (Phase 1c): see AgentTaskDao.hasRecentRunningTask()'s comment
    // for the full reasoning — this is the cross-activity-safe substitute
    // for AgentEngine.isTaskRunning(), which only checks an in-memory field
    // on whatever AgentEngine instance happens to own it. RecordingEngine
    // calls this before starting a new recording so a live agent task's
    // own automated taps/types can't get captured into the new macro.
    // 5 minutes is deliberately generous — a real in-progress task updates
    // its timestamp after every single step (see updateProgress above), so
    // an actually-running task never gets close to this window going
    // stale; a crashed one will, instead of blocking recording forever.
    suspend fun isAnyTaskRunning(): Boolean {
        val recentWindowMs = 5 * 60 * 1000L
        return dao.hasRecentRunningTask(System.currentTimeMillis() - recentWindowMs)
    }
