    ...
    suspend fun openApp(context: Context, packageName: String): ActionResult {
        val (ok, attempts) = retryUntil { _ ->
            try {
                ...
                if (!ScreenContextEngine.isAppInForeground(packageName)) {
        ...
        val result = if (ok) ActionResult(true, "Opened $packageName.")
        else ActionResult(false, "Couldn't confirm $packageName opened after $attempts attempt(s) — it may not be installed.")
