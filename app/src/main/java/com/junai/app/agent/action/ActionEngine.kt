...
object ActionEngine {
    ...
    suspend fun tapStep(
        ...
        // Identifier-based lookup exhausted — try the recorded position.
    ...
    suspend fun longPressStep(
        ...
        // Identifier-based lookup exhausted — try holding at the recorded position.
        if (boundsLeft != null && boundsTop != null && boundsRight != null && boundsBottom != null) {
            val cx = (boundsLeft + boundsRight) / 2f
            val cy = (boundsTop + boundsBottom) / 2f
    ...
    suspend fun scrollStep(
        ...
        val (ok, attempts) = retryUntil(maxRetries = 1) { _ ->
            val node = when {
                !resourceId.isNullOrBlank() -> svc.findNodeById(resourceId)
                !contentDescription.isNullOrBlank() -> svc.findNodeByText(contentDescription)
                else -> null
            }
            node?.let { svc.scroll(it, forward).also { _ -> it.recycle() } } ?: false
        }
        ...
        // No scrollable node found by id/description — replay the literal
        // swipe using the recorded bounds as the drag path.
        if (boundsLeft != null && boundsTop != null && boundsRight != null && boundsBottom != null) {
            val cx = (boundsLeft + boundsRight) / 2f
    ...
    suspend fun typeStep(
        ...
        val (ok, attempts) = retryUntil { _ ->
            val identifierNode = when {
                !resourceId.isNullOrBlank() -> svc.findNodeByIdDisambiguated(resourceId, contentDescription)
                !contentDescription.isNullOrBlank() -> svc.findNodeByText(contentDescription)
                else -> null
            }
            val node = identifierNode ?: run {
                if (boundsLeft != null && boundsTop != null && boundsRight != null && boundsBottom != null) {
                    val cx = (boundsLeft + boundsRight) / 2f
                    val cy = (boundsTop + boundsBottom) / 2f
        ...
        val result = if (ok) ActionResult(true, "Typed into \"$label\" (attempt $attempts).")
