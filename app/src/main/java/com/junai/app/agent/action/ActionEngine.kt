...
object ActionEngine {
    ...
    suspend fun scrollStep(
        ...
        val (ok, usedAlternate) = attemptOnMatch(match) { node -> svc.scroll(node, forward) }
        if (ok) {
            val result = ActionResult(true, "Scrolled \"$label\".")
            log("scrollStep", label, result)
            logStepOutcome(svc.applicationContext, macroId, stepIndex, "SWIPE", true, match.matchedVia, usedAlternate, null)
            return result
        }
