...
object ActionEngine {
    ...
    suspend fun tapStep(
        ...
        val (ok, attempts) = retryUntil { _ ->
            val node = svc.findBestMatchingNode(resourceId, text, contentDescription, className, boundsLeft, boundsTop, boundsRight, boundsBottom)
            node?.let { svc.tap(it).also { _ -> it.recycle() } } ?: false
        }
        if (ok) {
            val result = ActionResult(true, "Tapped \"$label\" (attempt $attempts).")
        ...
        val safeToUseBounds = packageName.isNullOrBlank() || ScreenContextEngine.isAppInForeground(packageName)
        ...
        val result = ActionResult(false, "Couldn't find or tap \"$label\" after $attempts attempt(s).")
        log("tapStep", label, result)
        return result
    }
    ...
    suspend fun longPressStep(
        ...
        if (ok) {
            val result = ActionResult(true, "Long-pressed \"$label\" (attempt $attempts).")
        ...
        val result = ActionResult(false, "Couldn't long-press \"$label\" after $attempts attempt(s).")
        log("longPressStep", label, result)
        return result
    }
    ...
    suspend fun scrollStep(
        ...
        val (ok, attempts) = retryUntil(maxRetries = 1) { _ ->
            val node = svc.findBestMatchingNode(resourceId, null, contentDescription, className, boundsLeft, boundsTop, boundsRight, boundsBottom)
            node?.let { svc.scroll(it, forward).also { _ -> it.recycle() } } ?: false
        }
        if (ok) {
            val result = ActionResult(true, "Scrolled \"$label\" (attempt $attempts).")
        ...
        if (safeToUseBounds && boundsLeft != null && boundsTop != null && boundsRight != null && boundsBottom != null) {
            ...
            val swiped = if (forward) svc.swipeAt(cx, bottomY, cx, topY) else svc.swipeAt(cx, topY, cx, bottomY)
        ...
        val result = ActionResult(false, "Couldn't scroll \"$label\" after $attempts attempt(s).")
        log("scrollStep", label, result)
        return result
    }
    ...
    suspend fun typeStep(
        ...
        val (ok, attempts) = retryUntil { _ ->
            val node = svc.findBestMatchingNode(resourceId, null, contentDescription, className, boundsLeft, boundsTop, boundsRight, boundsBottom)
                ?: run {
                    // BUGFIX: same wrong-app risk tapStep's bounds fallback
                    // has — only hit-test the recorded position if we've
                    // confirmed we're actually still in the recorded app,
                    // otherwise this could silently type into an unrelated
                    // field in whatever app happens to be on screen.
                    if (safeToUseBounds && boundsLeft != null && boundsTop != null && boundsRight != null && boundsBottom != null) {
                        val cx = (boundsLeft + boundsRight) / 2f
                        val cy = (boundsTop + boundsBottom) / 2f
                        svc.findNodeAtPosition(cx, cy)
                    } else null
                }
            if (node == null) return@retryUntil false
            if (node.isPassword) { node.recycle(); return@retryUntil false }
