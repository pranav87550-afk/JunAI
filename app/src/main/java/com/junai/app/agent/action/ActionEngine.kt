...
object ActionEngine {
    ...
    suspend fun tapStep(
        ...
        // PHASE D (checkbox/switch/radio idempotency): forwarded from
        // RecordedStep.expectedChecked — see that field's doc comment.
        ...
        // BUGFIX (repeated false "system_popup is covering the screen"
        // stops — third time this exact symptom showed up): the previous
        // fix tuned the overlay check's SIZE threshold, but the real bug
        // was structural, not a threshold problem. This ran BEFORE ever
        // trying to find the target, so it could block a tap on an
        // element that was perfectly reachable — like "Do Not Disturb"
        // INSIDE the Quick Settings panel itself. The QS panel is large
        // (comfortably over any reasonable size threshold) and IS a
        // TYPE_SYSTEM window, but it isn't blocking anything — it's the
        // panel the target lives in. No size threshold can fix that,
        // because size was never the right question; reachability is.
        // Finding the exact node below already proves nothing is in the
        // way — so that's what decides now, not a preemptive guess.
        ...
        // BUGFIX (checkbox/switch/radio idempotency): this tap was
        // recorded on a checkable element, and expectedChecked carries
        // what its state should be AFTER the tap. If the element is
        // ALREADY in that state when replay runs (macro ran before, or
        // the app simply started in that state), tapping it again would
        // flip it to the WRONG state — the same class of problem Phase A
        // solved for non-idempotent actions like Send, but for checkable
        // widgets specifically, where "already correct" is directly
        // checkable up front instead of only inferrable after the fact.
        if (expectedChecked != null && match.node?.isChecked == expectedChecked) {
            val result = ActionResult(true, "\"$label\" already ${if (expectedChecked) "on" else "off"} — skipped, no tap needed.")
            log("tapStep", label, result)
            logStepOutcome(svc.applicationContext, macroId, stepIndex, "TAP", true, match.matchedVia, false, null)
        ...
        //
        // BUGFIX (root cause of QS-tile taps failing even though the panel
        // WAS open — confirmed via on-device log, "Do Not Disturb"/"On"
        // tiles): ScreenContextEngine.isAppInForeground() reads currentApp,
        // which only updates from an accessibility event's packageName.
        // com.android.systemui's Quick Settings shade doesn't reliably fire
        // those events on this OEM, so this check could report "not in
        // foreground" even while the panel was genuinely open, blocking the
        // bounds fallback for no real reason. MacroReplayEngine's own
        // pre-step check already gets this right via
        // isQuickSettingsWindowOpen() (live window list, no event
        // dependency) — mirrored here so tapStep's internal safety check
        // agrees with it instead of using the stale event-based signal.
        val safeToUseBounds = packageName.isNullOrBlank() || if (packageName == "com.android.systemui") {
            JunAccessibilityService.instance?.isQuickSettingsWindowOpen() == true
        } else {
            ScreenContextEngine.isAppInForeground(svc.applicationContext, packageName)
        }
        ...
        // BUGFIX (see the reordering note above this function's top): the
        // overlay check now runs HERE — only once the target has
        // genuinely NOT been found — purely to add useful context to the
        // failure message, never to block a tap that would have
        // succeeded anyway.
        val overlay = ScreenContextEngine.currentBlockingOverlay()
        val result = if (overlay != null) {
            ActionResult(false, "Couldn't find or tap \"$label\" — a $overlay may be covering the screen.")
        } else {
            ActionResult(false, "Couldn't find or tap \"$label\".")
        }
        log("tapStep", label, result)
    ...
    suspend fun scrollStep(
        ...
        //
        // BUGFIX: same stale-foreground-check fix as tapStep — see that
        // function's doc comment. Matters just as much here: a QS
        // page-swipe recorded with no matchable node (see below) is exactly
        // the case that needs this fallback most.
        val safeToUseBounds = packageName.isNullOrBlank() || if (packageName == "com.android.systemui") {
            JunAccessibilityService.instance?.isQuickSettingsWindowOpen() == true
        } else {
            ScreenContextEngine.isAppInForeground(svc.applicationContext, packageName)
        ...
        // BUGFIX (root cause of QS page-swipes never replaying at all): a
        // swipe captured via RecordingEngine.captureScrollFromEventFallback
        // (the accessibility event had no source node — common for
        // Quick Settings tile-page paging on this OEM) has no resourceId
        // and no bounds, so neither fallback above can ever fire — this
        // step failed unconditionally before. We know the axis/direction
        // (scrollForward/scrollHorizontal, still recorded from the event's
        // own scrollDelta) and that we're safely on the recorded package
        // (safeToUseBounds above), just not the exact drag coordinates. A
        // swipe across the middle band of the CURRENT screen in that
        // direction is a reasonable generic substitute — it's how a person
        // would swipe a panel they can see but don't have precise
        // coordinates for.
        if (safeToUseBounds && !packageName.isNullOrBlank()) {
            ...
            val result = if (swiped) {
                ActionResult(true, "Swiped \"$label\" (koi exact position record nahi hui thi — screen ke beech se generic swipe kiya).")
            } else {
                ActionResult(false, "Couldn't scroll or swipe \"$label\" — koi position record nahi thi aur generic swipe bhi fail ho gaya.")
            }
            log("scrollStep", label, result)
            logStepOutcome(svc.applicationContext, macroId, stepIndex, "SWIPE", swiped, "generic_screen", false, if (swiped) null else "generic_swipe_failed")
            return result
        }
    ...
    /**
     * PHASE D (slider/SeekBar replay). Uses ACTION_SET_PROGRESS to set the
     * value directly in one call, rather than simulating a drag gesture
     * along the slider's track — deliberately, since a simulated drag
     * would depend on the slider rendering at the same on-screen size/
     * track length it had at record time (font scaling, display size, a
     * differently-sized notification-shade layout on another device all
     * change that), where a direct value-set does not.
     */
    suspend fun slideStep(
        resourceId: String?,
        text: String?,
        contentDescription: String?,
        targetValue: Float,
        ...
        val svc = service() ?: return noServiceResult().also { log("slideStep", resourceId ?: text ?: "?", it) }
        val label = resourceId ?: text ?: contentDescription ?: "slider"

        // BUGFIX (brightness slider "moves but doesn't do anything"): the
        // Quick Settings brightness slider on this device only updates its
        // own displayed progress via ACTION_SET_PROGRESS — the actual
        // screen brightness is driven by a separate path that this
        // accessibility action never reaches. Volume and most other
        // sliders don't have this problem; brightness specifically does,
        // so it's detected and routed through the real
        // Settings.System.SCREEN_BRIGHTNESS API instead (setBrightness()
        // already exists for this, used elsewhere for direct brightness
        // commands).
        //
        // Deliberately does NOT reuse setBrightness()'s existing
        // auto-navigate-to-grant-permission behavior here — that's the
        // right thing for a standalone "set brightness" command, but
        // auto-opening a settings screen in the MIDDLE of a multi-step
        // macro replay would yank the user away from whatever screen the
        // rest of the macro still needs, breaking every step after this
        // one. So: check permission first, and if it's missing, fail this
        // one step with a clear explanation instead of silently
        // navigating anywhere — the user grants it explicitly (Permission
        // Centre already has a WRITE_SETTINGS entry) rather than being
        // redirected mid-macro without warning.
        val isBrightnessSlider = listOfNotNull(resourceId, text, contentDescription)
            .any { it.contains("brightness", ignoreCase = true) }
        if (isBrightnessSlider && minValue != null && maxValue != null && maxValue > minValue) {
            if (!android.provider.Settings.System.canWrite(svc.applicationContext)) {
                val result = ActionResult(
                    false,
                    "\"$label\" ka slider move ho gaya, par asli brightness badalne ke liye \"Modify system settings\" permission chahiye — abhi ye granted nahi hai. Permission Centre se on karke dobara try karo."
                )
                log("slideStep", label, result)
                logStepOutcome(svc.applicationContext, macroId, stepIndex, "SLIDE", false, null, false, "write_settings_permission_missing")
                return result
            }
            val level = (((targetValue - minValue) / (maxValue - minValue)) * 255f).toInt()
            val result = setBrightness(svc.applicationContext, level)
            log("slideStep", label, result)
            logStepOutcome(svc.applicationContext, macroId, stepIndex, "SLIDE", result.success, "brightness_api", false, if (result.success) null else "set_brightness_failed")
            return result
        }
        ...
        if (node == null) {
            match.alternate?.let { try { it.recycle() } catch (e: Exception) { /* already gone */ } }
            val result = ActionResult(false, "Couldn't find slider \"$label\" to set its value.")
            log("slideStep", label, result)
            logStepOutcome(svc.applicationContext, macroId, stepIndex, "SLIDE", false, null, false, "not_found")
            return result
        }
        val ok = try {
            val args = android.os.Bundle()
            args.putFloat(AccessibilityNodeInfo.ACTION_ARGUMENT_PROGRESS_VALUE, targetValue)
            node.performAction(AccessibilityNodeInfo.AccessibilityAction.ACTION_SET_PROGRESS.id, args)
        } catch (e: Exception) { false }
        try { node.recycle() } catch (e: Exception) { /* already gone */ }
        match.alternate?.let { try { it.recycle() } catch (e: Exception) { /* already gone */ } }
        val result = if (ok) {
            ActionResult(true, "Set \"$label\" to $targetValue.")
        } else {
            ActionResult(false, "Couldn't set \"$label\" — the slider didn't accept the value.")
        }
        log("slideStep", label, result)
        logStepOutcome(svc.applicationContext, macroId, stepIndex, "SLIDE", ok, match.matchedVia, false, if (ok) null else "set_progress_failed")
        return result
    }
    ...
    suspend fun typeStep(
        ...
        // BUGFIX: same preemptive-gate bug as tapStep — see that
        // function's comment above its own match-finding call for the
        // full reasoning (a large legitimate system window, like the one
        // hosting the very field being typed into, was getting treated
        // as something blocking it). Removed here too rather than only
        // in tapStep, since the underlying reasoning is identical.
        val safeToUseBounds = packageName.isNullOrBlank() || ScreenContextEngine.isAppInForeground(svc.applicationContext, packageName)
