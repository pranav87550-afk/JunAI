...
object ActionEngine {
    ...
    suspend fun openApp(context: Context, packageName: String): ActionResult {
        ...
        // BUGFIX: some phones/apps put a fingerprint, PIN, or app-lock
        // prompt in front of the app right after launch (WhatsApp's own
        // "App Lock" privacy setting, MIUI App Lock, Secure Folder, etc).
        // isAppInForeground() only recognizes the target app's own package,
        // so a lock prompt (often a different package, e.g. system UI or
        // an OEM security app) read as "still not open" and the whole
        // action failed after a few seconds — even though the app WAS
        // opening and just needed the user to unlock it. If we detect a
        // lock-screen-looking prompt, wait much longer for the user to
        // actually enter it, instead of giving up in ~2 seconds.
        if (!ok && launched && ScreenContextEngine.looksLikeLockScreen()) {
            LockWaitBannerOverlay.show(context, "🔒 Waiting for you to unlock $packageName…")
            val unlocked = run {
                val deadline = System.currentTimeMillis() + 25_000L // give the user real time to unlock
                while (System.currentTimeMillis() < deadline) {
                    ...
                    LockWaitBannerOverlay.update("🔒 Waiting for you to unlock $packageName… (${secondsLeft}s)")
                    delay(500L)
                }
                false
            }
            LockWaitBannerOverlay.hide()
            result = if (unlocked) ActionResult(true, "Opened $packageName (waited for you to unlock it).")
            else ActionResult(false, "$packageName is locked and wasn't unlocked in time — unlock it and try again.")
        }
