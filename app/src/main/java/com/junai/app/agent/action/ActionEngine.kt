...
object ActionEngine {
    ...
    suspend fun tapStep(
        ...
        val node = retryFindNode {
            svc.findBestMatchingNode(resourceId, text, contentDescription, className, boundsLeft, boundsTop, boundsRight, boundsBottom, packageName)
        }
        val ok = node?.let { val r = svc.tap(it); it.recycle(); r } ?: false
        if (ok) {
    ...
    suspend fun typeStep(
        ...
        val (ok, attempts) = retryUntil { _ ->
            val node = svc.findBestMatchingNode(resourceId, null, contentDescription, className, boundsLeft, boundsTop, boundsRight, boundsBottom, packageName)
                ?: run {
