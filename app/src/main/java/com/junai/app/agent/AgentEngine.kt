    ...
    private suspend fun executeActionStep(step: AgentStep, params: IntentDetector.AgentTaskParams): ActionResult {
        return when (step.type) {
            ...
            StepType.TYPE -> {
                ...
                if (contact != null) {
                    // Step A: try search button (WhatsApp has one in the top bar)
                    val searchHints = listOf("Search", "Search…", "search")
                    for (hint in searchHints) {
                        if (ActionEngine.tap(hint).success) break
                    }
                    kotlinx.coroutines.delay(600)

                    // Step B: type the contact name into the search/find field
                    val searchFieldHints = listOf("Search…", "Search", "search_src_text")
                    for (hint in searchFieldHints) {
                        if (ActionEngine.typeText(hint, contact).success) break
                    }
                    kotlinx.coroutines.delay(1000) // wait for results to load

                    // Step C: tap on the contact in results
                    if (!ActionEngine.tap(contact).success) {
                        // Contact not found in search results
                        return ActionResult(
                            false,
                            "Opened WhatsApp but couldn't find \"$contact\" in your chats. Make sure their name is saved exactly as you said it."
                        )
                    }
                    kotlinx.coroutines.delay(800) // wait for chat to open
                }
