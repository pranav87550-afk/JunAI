    ...
    private suspend fun executeActionStep(step: AgentStep, params: IntentDetector.AgentTaskParams): ActionResult {
        return when (step.type) {
            ...
            StepType.TYPE -> {
                ...
                // meant the literal text "Mom ko message bhejo 'kal milte
                // hain'" got typed into the message box instead of just
                // "kal milte hain". Extract the quoted portion when present.
                ...
                if (quoted == null) {
                    ...
                    // Previously this fell back to typing the raw clause —
                    // literally "message mummy" — into the chat box. Don't
                    // guess at message content; say so plainly instead.
                    return ActionResult(
                        false,
