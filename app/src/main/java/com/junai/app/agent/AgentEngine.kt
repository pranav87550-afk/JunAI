...
/**
 * IMPROVEMENT (Phase 1f): runTask() used to return a bare String summary —
 * executeSteps() already computed a `succeeded: Boolean` internally (see
 * its own comment) but baked it into the string and never exposed it.
 * ChatIntentHandler had no way to know a task genuinely failed, so a
 * repeatedly-failing agent task could never be logged via logFailure()
 * and could never surface in the Pending tab. This wraps the same summary
 * text alongside that already-computed flag — no new logic, just exposing
 * what was already being calculated.
 */
data class AgentTaskResult(val summary: String, val succeeded: Boolean)
...
class AgentEngine(private val context: Context) {
     ...
     */
    suspend fun runTask(intentResult: IntentDetector.IntentResult): AgentTaskResult {
        ...
        if (isTaskRunning()) {
            return AgentTaskResult("I'm still working on something — say \"stop\" if you'd like me to cancel it first.", succeeded = false)
        }

        val params = intentResult.agentTaskParams
            ?: return AgentTaskResult("I'm not fully sure what you'd like me to do — can you rephrase that?", succeeded = false)
        ...
        if (params.actionType == IntentDetector.AgentActionType.RESUME) {
            val resumable = multiStepTaskManager.getResumableTask()
                ?: return AgentTaskResult("There's nothing paused right now to resume.", succeeded = false)
        ...
        if (steps.isEmpty()) {
            return AgentTaskResult("I couldn't figure out a clear plan for that — can you be more specific?", succeeded = false)
        }
    ...
    private suspend fun executeSteps(
        ...
        while (index < steps.size) {
            if (cancelRequested) {
                ...
                return AgentTaskResult("Okay, I stopped. So far: $progress Say \"resume\" anytime to continue.", succeeded = false)
            }
            ...
            when (decision.verdict) {
                DecisionVerdict.NEEDS_CLARIFICATION -> {
                    multiStepTaskManager.pauseTask(taskId)
                    currentTaskId = null
                    return AgentTaskResult(decision.reason ?: "I need a bit more detail before continuing — can you clarify?", succeeded = false)
                }
        ...
        return AgentTaskResult(buildSummary(succeeded, completedMessages, failureReason), succeeded)
        } finally {
