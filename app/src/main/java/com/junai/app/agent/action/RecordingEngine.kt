...
object RecordingEngine {
    ...
    fun captureSystemUiContentChange(context: Context, windows: List<android.view.accessibility.AccessibilityWindowInfo>) {
        ...
        for (node in candidates) {
            val rect = boundsOf(node)
            val key = "${node.viewIdResourceName}|${rect.left}|${rect.top}"
            val (label, desc) = labelOf(node)
        ...
        if (qsTileSnapshot.isNotEmpty()) {
            for ((key, signature) in newSnapshot) {
                ...
                rememberElement(context, node.packageName?.toString(), node.viewIdResourceName, node.className?.toString(), label ?: desc, rect)
                // One content-changed burst = one real tap. Stop after the
                // first tile whose state actually differs, so a cascade of
                // secondary UI updates (panel re-layout, other tiles'
                // dimming animations) triggered by that one tap doesn't
