...
object RecordingEngine {
    ...
    /**
     * BUGFIX ("86260 43662" → "86260 436622" bug): the very last keypress
     * of a dial-pad capture can have its two accessibility events arrive
     * in the OPPOSITE of the usual order — the digit field's text-change
     * (captured as a "Type" step, already showing the complete, correct
     * number) fires BEFORE that same press's click event (captured as a
     * "Tap" step), instead of after. Every OTHER digit in the same
     * recording relies on Tap-then-Type ordering without issue; it's
     * specifically the final press where this was observed flipping.
     *
     * normalize()'s echo-collapse can't catch this: the tap isn't a
     * duplicate of an EARLIER tap (there's only one click event for this
     * press), it's redundant with a TYPE step that already captured its
     * effect. Concretely: if the recording's LAST step is a single
     * dial-pad key tap, and the step immediately before it is a Type step
     * whose captured text already ends in exactly that key's digit, the
     * tap adds nothing new — replaying it would press that key an extra,
     * real time. Scoped deliberately narrow (only ever the trailing step,
     * only an exact suffix match) so it can't misfire on a genuine
     * mid-recording repeated digit.
     */
    private fun dropRedundantTrailingKeyTap(list: List<RecordedStep>): List<RecordedStep> {
        if (list.size < 2) return list
        val last = list.last()
        val prev = list[list.size - 2]
        if (last.actionType != "TAP" && last.actionType != "LONG_PRESS") return list
        if (prev.actionType != "TYPE") return list
        val keyChar = last.text?.substringBefore(",")?.trim().orEmpty()
        if (keyChar.isEmpty()) return list
        val typedTail = prev.typedText?.trim()?.takeLast(keyChar.length).orEmpty()
        return if (keyChar == typedTail) list.dropLast(1) else list
    }
    ...
    suspend fun stopAndSave(): RecordResult {
        ...
        val cleaned = dropRedundantTrailingKeyTap(normalize(steps, stepCapturedAt))
