...
object RecordingEngine {
    ...
    private fun normalize(raw: List<RecordedStep>, capturedAt: List<Long>): List<RecordedStep> {
        ...
        for (i in raw.indices) {
            val step = raw[i]
            val isBackToBackDuplicate = lastKeptIndex >= 0 && run {
                val prev = raw[lastKeptIndex]
                prev.actionType == step.actionType &&
                    ...
                    (capturedAt.getOrElse(i) { 0L } - capturedAt.getOrElse(lastKeptIndex) { 0L }) < DUPLICATE_ECHO_WINDOW_MS
            }
    ...
    suspend fun stopAndSave(): RecordResult {
        ...
        val cleaned = normalize(steps, stepCapturedAt)
