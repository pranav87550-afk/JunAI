...
object RecordingEngine {
    ...
    private fun performQsDiffCapture(context: Context) {
        ...
        if (qsTileSnapshot.isNotEmpty()) {
            // FIX (picking the RIGHT tile, not just the first one that
            // differs): a single content-changed burst can legitimately
            // contain more than one tile whose signature changed — e.g. a
            // live tile (Hotspot's connected-device count, a clock) redraws
            // in the same burst as the tile that was actually tapped.
            // "First found in traversal order" had no way to prefer the
            // real tap over an incidental redraw that merely happened to
            // come first in the tree. A tile's CHECKED state flipping is
            // much stronger evidence of a genuine tap than its label text
            // changing for some other reason — so scan every differing
            // tile first, and prefer one whose checked-state actually
            // toggled; only fall back to the first differing entry if none
            // of them show a checked-state change (covers non-toggle tiles
            // like Screenshot, which don't have an on/off state at all).
            ...
            for ((key, signature) in newSnapshot) {
                ...
                if (fallbackKey == null) { fallbackKey = key; fallbackNode = node }
            ...
            val finalKey = chosenKey ?: fallbackKey
            val finalNode = chosenNode ?: fallbackNode
            if (finalKey != null && finalNode != null) {
    ...
    /** Reset between recording sessions so a stale snapshot from a previous "sikhao" never gets diffed against a completely different one. */
