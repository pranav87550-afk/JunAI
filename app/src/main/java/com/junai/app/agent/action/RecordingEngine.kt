...
object RecordingEngine {
    ...
    /**
     * PHASE F (SystemSurfaceRegistry — first extension beyond the home
     * screen). Same reasoning as HOME_SCREEN_SENTINEL: Recents/Overview
     * is hosted by different packages across OEMs and Android versions
     * (bundled into the launcher on stock/Pixel-style builds, a separate
     * systemui-owned window on others) — a literal package string
     * recorded on one device would never match on another. This
     * substitutes a portable sentinel instead, resolved fresh at replay
     * time via JunAccessibilityService.openRecents() regardless of which
     * package actually owns it on THAT device.
     *
     * Detection heuristic, stated plainly rather than presented as more
     * certain than it is: a window whose className contains "Recents" or
     * "Overview" (case-insensitive) — matches AOSP's own naming and most
     * OEM skins that follow it, but isn't verified across every OEM.
     * Needs on-device confirmation, same as the blocking-overlay size
     * heuristic did — flagging rather than assuming.
     */
    const val RECENTS_SENTINEL = "<recents_screen>"
     ...
     */
    /**
     * BUGFIX (root cause of QS tile macros literally naming themselves
     * "Tap On" / "Tap Off" — confirmed via user screenshot: a fresh
     * recording still showed steps labeled that way): a QS tile's own
     * text is frequently just its current state word ("On"/"Off"), not
     * its name — this device's tiles show a name label above and the
     * state word below as a SEPARATE text node, so the walk below can
     * reach the state word first and stop there, never continuing on to
     * the actual name. That's a transient value (flips every tap) rather
     * than a stable identifier, so replay matching by literal text "On"
     * silently fails the moment the tile's real state differs (see
     * ActionEngine.tapStep's "\"On\" nahi mila" case). Treat a bare
     * on/off word as a last-resort fallback, not an answer to stop on —
     * keep walking for a real name, and only fall back to the word itself
     * if nothing better turns up anywhere in the walk.
     */
    private fun isGenericStateWord(s: String?): Boolean {
    ...
    internal fun labelOf(node: AccessibilityNodeInfo): Pair<String?, String?> {
        ...
        while (n != null && depth < 5) {
            val rawText = n.text?.toString()?.takeIf { it.isNotBlank() }
            ...
            val rawDesc = n.contentDescription?.toString()?.takeIf { it.isNotBlank() }
                ?: (if (android.os.Build.VERSION.SDK_INT >= 30) n.stateDescription?.toString()?.takeIf { it.isNotBlank() } else null)
            if (rawText != null || rawDesc != null) {
                val genericText = isGenericStateWord(rawText)
                val genericDesc = isGenericStateWord(rawDesc)
                if (!genericText || !genericDesc) {
                    // At least one side is a real label (or only one side
                    // was present and it wasn't generic) — good enough,
                    // stop here as before.
                    return (rawText.takeUnless { genericText }) to (rawDesc.takeUnless { genericDesc })
                }
                // Both sides were bare "on"/"off" — remember as a
                // last-resort fallback, but keep walking for a real name.
                if (fallbackText == null && fallbackDesc == null) { fallbackText = rawText; fallbackDesc = rawDesc }
            }
            ...
            if (parent != null) {
                for (i in 0 until parent.childCount) {
                    ...
                    sibling.recycle()
                    if (sibText != null || sibDesc != null) {
                        val genericSibText = isGenericStateWord(sibText)
                        val genericSibDesc = isGenericStateWord(sibDesc)
                        if (!genericSibText || !genericSibDesc) {
                            return (sibText.takeUnless { genericSibText }) to (sibDesc.takeUnless { genericSibDesc })
                        }
                        if (fallbackText == null && fallbackDesc == null) { fallbackText = sibText; fallbackDesc = sibDesc }
                    }
                }
            }
            n = parent
        ...
        // Nothing better anywhere in the walk — the bare state word is
        // still more useful than nothing (label = null), so use it as a
        // last resort rather than losing the step's identity entirely.
        return fallbackText to fallbackDesc
    }

    /**
     * BUGFIX (confirmed on-device via the diagnostic Toast, not guessed):
     * Quick Settings tile taps deliver TYPE_VIEW_CLICKED with
     * event.source == null on this device — a known quirk where some OEM
     * system-window implementations don't attach a retrievable node to
     * their accessibility events. captureTap() structurally requires a
     * node, so every one of these taps was being silently dropped
     * entirely — this is the actual reason "kuch record nahi hua" in
     * Quick Settings specifically, not a capture-logic bug like the
     * earlier ones.
     *
     * Fallback: AccessibilityEvent itself still carries className,
     * contentDescription, and text directly — independent of the source
     * node. Degrade to those instead of discarding the tap. There's no
     * resourceId or bounds this way (neither is exposed on the event
     * itself, only via a node), so this step relies purely on
     * text/contentDescription/className matching at replay — a real but
     * strictly weaker signal than a full node capture. Deliberately does
     * NOT invent a resourceId, so Phase B's identity-signal requirement
     * at replay still applies honestly rather than being spoofed.
     */
    fun captureTapFromEventFallback(event: android.view.accessibility.AccessibilityEvent, context: Context) {
        ...
        if (ScreenContextEngine.looksLikeSensitiveLabel(text) || ScreenContextEngine.looksLikeSensitiveLabel(desc)) return
        if (text == null && desc == null) {
            // Genuinely nothing to identify this step by at all — even the
            // event-level fallback fields are empty. Recording it would
            // only produce an unmatchable step at replay, so skip it
            // rather than add noise; the diagnostic Toast already told
            // Pranav this tap was lost.
            return
        }
        ...
        addStep(
            RecordedStep(
                actionType = "TAP",
                packageName = packageNameForStep(event.packageName?.toString(), context),
                resourceId = null,
                text = text,
    ...
    private val sliderDebounceHandler = android.os.Handler(android.os.Looper.getMainLooper())
    private var sliderDebounceRunnable: Runnable? = null
    private const val SLIDER_SETTLE_DELAY_MS = 500L

    private data class PendingSlider(
        val packageName: String?, val resourceId: String?, val text: String?,
        val contentDescription: String?, val className: String?,
        val boundsLeft: Int, val boundsTop: Int, val boundsRight: Int, val boundsBottom: Int,
        val value: Float, val min: Float, val max: Float
    )
    private var pendingSlider: PendingSlider? = null

    /**
     * PHASE D (slider/SeekBar capture + replay — brightness, volume, and
     * similar drag-controlled UI). Called from JunAccessibilityService
     * whenever an event's source node reports a non-null RangeInfo — the
     * standard Android signal for "this is a slider-type control,"
     * independent of className, so it covers custom/Compose sliders too,
     * not just android.widget.SeekBar specifically.
     *
     * Debounced the same way as the Quick Settings tile-state capture,
     * and for the same reason: dragging a slider fires many intermediate
     * progress events (every pixel of movement can produce one), and only
     * the FINAL settled value is worth recording — capturing every tick
     * would produce dozens of near-duplicate steps for a single drag.
     * Unlike the QS tile case, no node needs to be held across the delay
     * here: the (value, identity) snapshot is read synchronously right
     * now and stored as plain data below, so there's no stale-
     * AccessibilityNodeInfo risk when the debounced commit later fires.
     */
    fun captureSliderChange(node: AccessibilityNodeInfo, context: Context) {
        if (!recordingActive) return
        val range = node.rangeInfo ?: return
        if (isSensitive(node)) return
        val (label, desc) = labelOf(node)
        if (label.isNullOrBlank() && desc.isNullOrBlank() && node.viewIdResourceName.isNullOrBlank()) return
        val rect = boundsOf(node)
        pendingSlider = PendingSlider(
            packageName = packageNameForStep(node.packageName?.toString(), context),
            resourceId = node.viewIdResourceName,
            text = label,
            contentDescription = desc,
            className = node.className?.toString(),
            boundsLeft = rect.left, boundsTop = rect.top, boundsRight = rect.right, boundsBottom = rect.bottom,
            value = range.current, min = range.min, max = range.max
        )
        sliderDebounceRunnable?.let { sliderDebounceHandler.removeCallbacks(it) }
        val runnable = Runnable { commitPendingSlider() }
        sliderDebounceRunnable = runnable
        sliderDebounceHandler.postDelayed(runnable, SLIDER_SETTLE_DELAY_MS)
    }

    private fun commitPendingSlider() {
        if (!recordingActive) return
        val p = pendingSlider ?: return
        pendingSlider = null
        addStep(
            RecordedStep(
                actionType = "SLIDE",
                ...
                boundsTop = p.boundsTop,
                boundsRight = p.boundsRight,
                boundsBottom = p.boundsBottom,
                sliderValue = p.value,
                sliderMin = p.min,
                sliderMax = p.max
            )
    ...
    /**
     * PHASE D (system-nav: Home). Called when the foreground window
     * transitions to the device's own launcher package while recording —
     * the standard, reliable signal that Home was pressed (button or
     * gesture), regardless of which the device uses; there's no need to
     * distinguish the two, since both land here the same way. Reuses
     * HOME_SCREEN_SENTINEL, already the recognized "this step means the
     * home screen" marker in the replay path.
     *
     * Deliberately does NOT try to capture Back/Recents/App-switch the
     * same way — the equivalent signal for those isn't as clean: Back
     * can land on any of dozens of different destinations depending on
     * the app's own navigation stack (no single foreground-package check
     * identifies it), and gesture-navigation devices don't reliably
     * deliver a distinguishable signal for Back or Recents to an
     * accessibility service the way Home's launcher transition works.
     * REPLAY of Back/Recents already works today via
     * JunAccessibilityService.pressBack()/openRecents() — only capturing
     * them from real usage is the open problem, and isn't solved here.
     */
    /**
     * PHASE F (SystemSurfaceRegistry: Recents). Called when the foreground
     * window's className matches the Recents/Overview naming pattern — see
     * RECENTS_SENTINEL's doc comment for the heuristic and its caveat.
     * Mirrors captureHomePress()'s structure exactly (same duplicate-
     * transition guard, same echo window).
     */
    fun captureRecentsOpen(context: Context) {
        if (!recordingActive) return
        if (steps.isEmpty()) return
        if (steps.last().actionType == "RECENTS") return
        if (System.currentTimeMillis() - lastTapAt < TAP_ECHO_WINDOW_MS) return
        lastTapAt = System.currentTimeMillis()
        addStep(RecordedStep(actionType = "RECENTS", packageName = RECENTS_SENTINEL, resourceId = null, text = null, contentDescription = null, className = null))
    }

    fun captureHomePress(context: Context) {
        if (!recordingActive) return
        // Starting a recording while already sitting on the home screen
        // isn't a "press home" action — nothing to capture yet.
        if (steps.isEmpty()) return
        // Already-recorded Home as the last step — this is very likely
        // the SAME transition firing WINDOW_STATE_CHANGED more than once
        // (common on Android), not the user pressing Home twice in a row.
        if (steps.last().actionType == "HOME") return
        if (System.currentTimeMillis() - lastTapAt < TAP_ECHO_WINDOW_MS) return
        lastTapAt = System.currentTimeMillis()
        addStep(RecordedStep(actionType = "HOME", packageName = HOME_SCREEN_SENTINEL, resourceId = null, text = null, contentDescription = null, className = null))
    }
    ...
    fun captureTap(node: AccessibilityNodeInfo, context: Context) {
        ...
        addStep(
            RecordedStep(
                ...
                expectedChecked = if (node.isCheckable) node.isChecked else null
            )
        )
    ...
    /**
     * BUGFIX (root cause of "Quick Settings mein kam/null record ho raha
     * hai" persisting even after captureTapFromEventFallback() above):
     * that fallback only helps when TYPE_VIEW_CLICKED fires at all, just
     * with a null source. On several OEM SystemUI builds (confirmed on
     * Pranav's own OPPO/ColorOS device), tapping a QS tile fires NO
     * click-type event whatsoever — the OEM's reimplemented tile view
     * drives the tap straight into its click handler without ever
     * routing through performClick()/sendAccessibilityEvent(), so there
     * is no click signal to hang capture off of at all. What DOES still
     * fire, because the tile visibly changes (icon/label/on-off state),
     * is a plain TYPE_WINDOW_CONTENT_CHANGED on the systemui window.
     *
     * Instead of waiting for a click, this snapshots every tile-like
     * node's identity+state each time systemui's content changes, and
     * diffs against the previous snapshot — whichever tile's signature
     * changed between two snapshots is the one that got tapped, the same
     * way you'd notice which light switch flipped rather than watching
     * the hand that flipped it. Skipped entirely if a real click was
     * already captured moments ago (TAP_ECHO_WINDOW_MS), so devices where
     * the normal click path DOES work never get a duplicate step.
     */
    private var qsTileSnapshot: Map<String, String> = emptyMap()
    private val qsDebounceHandler = android.os.Handler(android.os.Looper.getMainLooper())
    private var qsDebounceRunnable: Runnable? = null
    private const val QS_SETTLE_DELAY_MS = 700L
    private val qsPollHandler = android.os.Handler(android.os.Looper.getMainLooper())
    private var qsPollRunnable: Runnable? = null
    private const val QS_POLL_INTERVAL_MS = 450L

    /**
     * BUGFIX (root cause of "QS taps/swipes miss randomly, sometimes the
     * panel itself doesn't seem captured at all"): captureSystemUiContentChange()
     * — and everything downstream of it, including performQsDiffCapture()
     * below — only ever runs when JunAccessibilityService.onAccessibilityEvent
     * actually receives a TYPE_WINDOW_CONTENT_CHANGED event with
     * packageName == "com.android.systemui". On this OEM's SystemUI (ColorOS),
     * that event does NOT reliably fire for every Quick Settings tile change
     * or page swipe — sometimes it fires, sometimes it's batched/dropped
     * entirely, with no error, just silence. That's exactly the intermittent
     * "kabhi tap nahi hota, kabhi swipe nahi hota" pattern: it was never a
     * bug in the diffing logic itself (that part is solid — see the
     * checked-state and candidate-count fallbacks below), it's that the
     * function doing the diffing was simply never being called.
     *
     * Fix: stop depending on any specific accessibility event firing at all.
     * While recording is active, this polls performQsDiffCapture() on a
     * plain timer instead — cheap presence check every tick (just "is a
     * systemui window currently showing"), full tree-walk diff only when
     * one is. The event-triggered path in captureSystemUiContentChange() is
     * left in place too (harmless — it's just an extra, earlier poll tick
     * when the event DOES happen to fire), but this timer is now the
     * reliable backbone that doesn't care whether SystemUI cooperates.
     */
    private fun startQsPolling() {
        stopQsPolling()
        val runnable = object : Runnable {
            override fun run() {
                ...
                if (svc != null) {
                    val windows = try { svc.windows } catch (e: Exception) { null }
                    val qsOpen = windows?.any {
                        it.root?.packageName?.toString() == "com.android.systemui"
                    } == true
                    if (qsOpen) {
                        try { performQsDiffCapture(svc.applicationContext) } catch (e: Exception) { /* keep polling regardless */ }
                    }
                }
                qsPollHandler.postDelayed(this, QS_POLL_INTERVAL_MS)
            }
        }
        qsPollRunnable = runnable
        qsPollHandler.postDelayed(runnable, QS_POLL_INTERVAL_MS)
    }

    private fun stopQsPolling() {
        qsPollRunnable?.let { qsPollHandler.removeCallbacks(it) }
        qsPollRunnable = null
    }

    /**
     * BUGFIX (Hotspot-style tiles captured as several taps instead of
     * one — "1. Tap On / 2. Tap Off / 3. Tap On" from a SINGLE real tap):
     * some tiles (Hotspot, Bluetooth — anything doing real radio/
     * connection work) run their OWN internal sequence of UI states for
     * one user tap: Off → "Turning on..." → On — and each state change
     * fires its own TYPE_WINDOW_CONTENT_CHANGED burst, spaced MORE than
     * TAP_ECHO_WINDOW_MS apart since the transition genuinely takes over
     * a second. Reacting immediately to every burst captured each
     * intermediate state as its own separate step.
     *
     * Fix: debounce instead of reacting immediately — every new burst
     * cancels and reschedules the actual diff+capture, so it only runs
     * once the tile's state has actually stopped changing for
     * QS_SETTLE_DELAY_MS. That collapses "Off → Turning on → On" into a
     * single step based on the FINAL settled state, the way a person
     * watching would say "the tile turned on" rather than narrating every
     * intermediate frame. Deliberately re-fetches a FRESH windows/node
     * list when the debounced callback actually fires (performQsDiffCapture)
     * instead of holding the nodes passed in here across the delay —
     * AccessibilityNodeInfo references aren't safe to keep across an
     * arbitrary wait, they can go stale.
     */
    fun captureSystemUiContentChange(context: Context, windows: List<android.view.accessibility.AccessibilityWindowInfo>) {
        if (!recordingActive) return
        if (System.currentTimeMillis() - lastTapAt < TAP_ECHO_WINDOW_MS) return
        qsDebounceRunnable?.let { qsDebounceHandler.removeCallbacks(it) }
        val runnable = Runnable { performQsDiffCapture(context) }
        qsDebounceRunnable = runnable
        qsDebounceHandler.postDelayed(runnable, QS_SETTLE_DELAY_MS)
    }

    private fun performQsDiffCapture(context: Context) {
        if (!recordingActive) return
        val svc = JunAccessibilityService.instance ?: return
        val windows = try { svc.windows } catch (e: Exception) { return }

        val root = windows.asSequence()
            .mapNotNull { it.root }
            .firstOrNull { it.packageName?.toString() == "com.android.systemui" } ?: return

        val candidates = mutableListOf<AccessibilityNodeInfo>()
        collectTileCandidates(root, candidates, 0)
        if (candidates.isEmpty()) return

        val newSnapshot = LinkedHashMap<String, String>()
        val nodeByKey = HashMap<String, AccessibilityNodeInfo>()
        candidates.forEachIndexed { index, node ->
            // FIX (key stability): the original key included the tile's
            // absolute pixel position, which changes on minor re-layout —
            // an icon animating, a panel-open ripple settling, even
            // another tile's dimming transition — without the tile
            // actually having moved in the grid. That made the SAME tile
            // look like a brand-new one between snapshots, so its real
            // state change could be missed entirely. Traversal-order index
            // is far steadier: it only shifts if tiles are genuinely
            // reordered (some OEMs do recency-based reordering — a real,
            // separate edge case this doesn't solve, but the common case
            // of "nothing moved, something just redrew" is fixed).
            val key = "${node.viewIdResourceName}|idx$index"
            val (label, desc) = labelOf(node)
            val stateDesc = if (android.os.Build.VERSION.SDK_INT >= 30) node.stateDescription?.toString() else null
            newSnapshot[key] = "$label|$desc|$stateDesc|${node.isChecked}"
            nodeByKey[key] = node
        }

        // First snapshot of this panel session — nothing to diff against
        // yet, just establish the baseline. (Also naturally covers the
        // panel just having opened, where every tile would otherwise
        // "differ" from an empty snapshot and falsely look tapped.)
        if (qsTileSnapshot.isNotEmpty()) {
            // BUGFIX (false capture on a plain swipe/incidental touch that
            // never actually toggled anything): the old fallback treated
            // ANY differing entry as a valid tap when no checked-state
            // flip was found — meant for momentary-action tiles like
            // Screenshot that have no on/off state at all. In practice it
            // fired on things that were never a real tap: a swipe between
            // QS pages reusing the same recycled view (same resourceId +
            // traversal index, different underlying tile — see the
            // key-stability note above) looks exactly like "some entry's
            // signature differs" without any button ever being toggled.
            // A tile's CHECKED state actually flipping is the one signal
            // that's genuinely proof of a real tap — nothing else reliably
            // is. So this now requires that, full stop, no fallback.
            //
            // Known trade-off from before, now partly closed below: a
            // non-toggle "momentary action" tile (Screenshot, and
            // similar) tapped on a device where clicks never fire at all
            // still won't be captured by this path — no candidate-count
            // signal for it either. Flagging rather than guessing.
            var chosenKey: String? = null
            var chosenNode: AccessibilityNodeInfo? = null
            // BUGFIX (root cause of "quick panel replay turns tiles to the
            // WRONG state" — recorded ON, replay flips it OFF because the
            // tile was already ON): this loop is the only place that
            // actually knows the tile's checked state genuinely flipped
            // (oldChecked != newChecked below), and what it flipped TO.
            // That settled end-state used to be computed here and then
            // discarded — never carried into the RecordedStep — so replay
            // (ActionEngine.tapStep's expectedChecked idempotency check,
            // same guard captureTap() already gets for ordinary checkable
            // views) never had anything to check against for QS tiles
            // specifically, and just blind-tapped every time. This flag
            // remembers that the match came from a REAL checked-state
            // flip (not the count-based expand/collapse fallback below,
            // which isn't a toggle at all) so the tap-building block can
            // set expectedChecked only when it's actually meaningful.
            var chosenIsCheckedToggle = false
            for ((key, signature) in newSnapshot) {
                val oldSignature = qsTileSnapshot[key] ?: continue
                if (oldSignature == signature) continue
                val node = nodeByKey[key] ?: continue
                if (isSensitive(node)) continue
                val oldChecked = oldSignature.substringAfterLast('|')
                val newChecked = signature.substringAfterLast('|')
                if (oldChecked != newChecked) { chosenKey = key; chosenNode = node; chosenIsCheckedToggle = true; break }
            }
            // BUGFIX (dropdown/expand-arrow tiles — e.g. Wi-Fi's network
            // name, Hotspot's expand chevron — not captured at all): these
            // taps don't flip any CHECKED state, they expand/collapse a
            // detail panel, which the strict checked-state-only rule above
            // has no way to see. Re-enable a fallback for this — but
            // gated specifically on the total candidate COUNT changing
            // (rows genuinely added or removed), not "any signature
            // differs" the way the old, too-eager fallback was. A plain
            // swipe/incidental touch changes labels/positions but doesn't
            // add or remove actual tree nodes; an expand/collapse
            // genuinely does. That structural difference is real,
            // specific evidence — unlike the vague "something differs"
            // check this replaced originally.
            if (chosenKey == null && candidates.size != qsTileSnapshot.size) {
                for ((key, signature) in newSnapshot) {
                    val oldSignature = qsTileSnapshot[key] ?: continue
                    if (oldSignature == signature) continue
                    val node = nodeByKey[key] ?: continue
            ...
            if (chosenKey != null && chosenNode != null) {
                val finalNode = chosenNode
                val rect = boundsOf(finalNode)
                val (label, desc) = labelOf(finalNode)
                lastTapAt = System.currentTimeMillis()
                addStep(
                    RecordedStep(
                        actionType = "TAP",
                        packageName = packageNameForStep(finalNode.packageName?.toString(), context),
                        resourceId = finalNode.viewIdResourceName,
                        text = label,
                        contentDescription = desc,
                        className = finalNode.className?.toString(),
                        boundsLeft = rect.left,
                        boundsTop = rect.top,
                        boundsRight = rect.right,
                        boundsBottom = rect.bottom,
                        idDistinctive = finalNode.viewIdResourceName?.let { idSiblingCount(finalNode) <= 1 },
                        // Only set for a genuine checked-state flip (see
                        // chosenIsCheckedToggle above) — the expand/collapse
                        // fallback isn't a toggle, so it stays null there,
                        // same as it always has.
                        expectedChecked = if (chosenIsCheckedToggle) finalNode.isChecked else null
                    )
                )
                rememberElement(context, finalNode.packageName?.toString(), finalNode.viewIdResourceName, finalNode.className?.toString(), label ?: desc, rect)
                // One content-changed burst = one real tap. Every OTHER
                // differing tile in this same burst is treated as
                // incidental churn caused by that one tap (panel re-layout,
                // other tiles' dimming animations), not a second tap.
            } else if (candidates.size == qsTileSnapshot.size) {
                // BUGFIX (QS page swipe never captured as anything): a
                // swipe between Quick Settings pages reuses the same
                // recycled views (same resourceId + traversal index) with
                // entirely different tiles underneath — see the
                // key-stability note above. That means MOST entries differ
                // at once, none of them via a checked-state flip, and the
                // candidate count stays the same (still a full page of
                // tiles) — a signature distinct from both a real tap
                // (exactly one entry differs) and a dropdown/expand
                // (count changes). Previously this whole case just fell
                // through and got silently absorbed into the new baseline
                // snapshot below — a real swipe the user did, producing no
                // recorded step at all. Now it's recognized on its own
                // structural signature and recorded as a horizontal swipe
                // on the panel, replayed via ActionEngine.scrollStep's
                // raw-gesture fallback (QS pagers rarely support
                // ACTION_SCROLL_FORWARD/BACKWARD, so this is expected to
                // land on the bounds-based drag, not the node-scroll path).
                var differingCount = 0
                for ((key, signature) in newSnapshot) {
                    val oldSignature = qsTileSnapshot[key] ?: continue
                    if (oldSignature != signature) differingCount++
                }
                if (differingCount >= (candidates.size / 2).coerceAtLeast(2)) {
                    val panelRoot = windows.asSequence()
                        .mapNotNull { it.root }
                        .firstOrNull { it.packageName?.toString() == "com.android.systemui" }
                    if (panelRoot != null) {
                        val rect = boundsOf(panelRoot)
                        lastTapAt = System.currentTimeMillis()
                        addStep(
                            RecordedStep(
                                actionType = "SWIPE",
                                packageName = packageNameForStep("com.android.systemui", context),
                                resourceId = panelRoot.viewIdResourceName,
                                text = null,
                                contentDescription = null,
                                className = panelRoot.className?.toString(),
                                boundsLeft = rect.left,
                                boundsTop = rect.top,
                                boundsRight = rect.right,
                                boundsBottom = rect.bottom,
        ...
        qsTileSnapshot = newSnapshot
        for (n in candidates) { try { n.recycle() } catch (e: Exception) { /* already gone */ } }
    }

    /** Reset between recording sessions so a stale snapshot from a previous "sikhao" never gets diffed against a completely different one. Also clears any pending (not yet committed) slider drag — same reasoning, a debounced commit from a previous session should never land in a new one. */
    private fun resetQsTileSnapshot() {
        qsTileSnapshot = emptyMap()
        qsDebounceRunnable?.let { qsDebounceHandler.removeCallbacks(it) }
        qsDebounceRunnable = null
        sliderDebounceRunnable?.let { sliderDebounceHandler.removeCallbacks(it) }
        sliderDebounceRunnable = null
        pendingSlider = null
    }

    private fun collectTileCandidates(node: AccessibilityNodeInfo, out: MutableList<AccessibilityNodeInfo>, depth: Int) {
        if (depth > 12) return
        ...
        for (i in 0 until node.childCount) {
            ...
            collectTileCandidates(child, out, depth + 1)
            try { child.recycle() } catch (e: Exception) { /* already gone */ }
        }
    }
    ...
    /**
     * BUGFIX (root cause of QS panel page-swipes never getting captured at
     * all — confirmed on-device via the diagnostic Toast in
     * JunAccessibilityService: "swipe had no source (pkg=com.android.
     * systemui) — step skipped"): TYPE_VIEW_SCROLLED sometimes fires with
     * no source node — same underlying gap TAP already has a fallback for
     * (captureTapFromEventFallback above), SWIPE never did. No node means
     * no resourceId and no bounds to record; ActionEngine.scrollStep()'s
     * generic-screen-swipe fallback (see its own doc comment) is what
     * makes this replayable anyway — it only needs the axis/direction,
     * which the event's own scrollDelta still gives us, and the package
     * name, for the same "don't swipe a different app" safety check
     * tapStep/scrollStep already do.
     */
    fun captureScrollFromEventFallback(event: android.view.accessibility.AccessibilityEvent, forward: Boolean, horizontal: Boolean, context: Context) {
        if (!recordingActive) return
        if (System.currentTimeMillis() - lastTapAt < TAP_ECHO_WINDOW_MS) return
        val pkg = packageNameForStep(event.packageName?.toString(), context)
        // Same debounce reasoning as captureScroll(): collapse repeated
        // events from one physical swipe into the last step instead of
        // adding a new one each time, matched on package+direction since
        // there's no resourceId here to match on.
        val last = steps.lastOrNull()
        if (last != null && last.actionType == "SWIPE" && last.resourceId == null &&
            last.packageName == pkg && last.scrollForward == forward && last.scrollHorizontal == horizontal
        ) {
            return
        }
        addStep(
    ...
    private fun dropKeypadTapsAdjacentToType(list: List<RecordedStep>): List<RecordedStep> {
        ...
        for (i in list.indices) {
            val step = list[i]
            // BUGFIX (fix never actually firing): dialer keys have a
            // BLANK step.text — their "6,MNO"-style label lives in
            // contentDescription instead (see labelOf()'s doc comment:
            // text is checked first, but for these buttons it's empty,
            // so contentDescription is what actually got captured).
            // Checking only step.text here meant isKeypadKeyTap was
            // ALWAYS false, silently, and this entire fix never removed
            // a single step — which is exactly why the same "extra
            // digit" bug kept reproducing even after this landed. Check
            // whichever field is actually non-blank, same fallback order
            // buildSummary() itself already uses.
            val keyLabel = step.text?.takeIf { it.isNotBlank() } ?: step.contentDescription
            val isKeypadKeyTap = step.actionType == "TAP" &&
                keyLabel?.trim()?.let { KEYPAD_KEY_PATTERN.matches(it) } == true
            if (isKeypadKeyTap) {
    ...
    private fun buildSummary(list: List<RecordedStep>): List<String> {
        return list.mapIndexed { index, step ->
            ...
            val action = when (step.actionType) {
                ...
                "SLIDE" -> {
                    val pct = if (step.sliderMin != null && step.sliderMax != null && step.sliderMax != step.sliderMin && step.sliderValue != null) {
                        (((step.sliderValue - step.sliderMin) / (step.sliderMax - step.sliderMin)) * 100).toInt()
                    } else null
                    if (pct != null) "Set $label to $pct%" else "Set $label"
                }
    ...
    fun serializeSteps(list: List<RecordedStep>): String {
        ...
        for (s in list) {
            arr.put(JSONObject().apply {
                ...
                put("sliderValue", s.sliderValue ?: JSONObject.NULL)
                put("sliderMin", s.sliderMin ?: JSONObject.NULL)
                put("sliderMax", s.sliderMax ?: JSONObject.NULL)
                put("expectedChecked", s.expectedChecked ?: JSONObject.NULL)
            })
        }
        return arr.toString()
    }
        ...
        return try {
            ...
            (0 until arr.length()).map { i ->
                ...
                RecordedStep(
                    ...
                    sliderValue = if (o.has("sliderValue") && !o.isNull("sliderValue")) o.optDouble("sliderValue").toFloat() else null,
                    sliderMin = if (o.has("sliderMin") && !o.isNull("sliderMin")) o.optDouble("sliderMin").toFloat() else null,
                    sliderMax = if (o.has("sliderMax") && !o.isNull("sliderMax")) o.optDouble("sliderMax").toFloat() else null,
                    expectedChecked = if (o.has("expectedChecked") && !o.isNull("expectedChecked")) o.optBoolean("expectedChecked") else null
                )
            }
        } catch (e: Exception) {
            emptyList()
