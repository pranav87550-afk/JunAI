...
object RecordingEngine {
    ...
    private fun performQsDiffCapture(context: Context) {
        ...
        if (qsTileSnapshot.isNotEmpty()) {
            ...
            //
            // Known trade-off, on purpose: a non-toggle "momentary action"
            // tile (Screenshot, and similar) tapped on a device where
            // clicks never fire at all won't be captured by this path
            // anymore. That's a real capability gap, not silently
            // papered over — flagging it rather than guessing at a
            // fallback heuristic that (as this bug shows) is too easy to
            // trigger falsely.
