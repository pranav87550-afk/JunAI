...
object RecordingEngine {
    ...
    /** Outcome of stopAndSave — Empty when nothing usable was captured, Done with the saved/updated macro otherwise. */
    sealed class RecordResult {
        object Empty : RecordResult()
        data class Done(val macro: RecordedMacroEntity, val duplicatesRemoved: Int) : RecordResult()
    }
     ...
     */
    fun captureScroll(node: AccessibilityNodeInfo, forward: Boolean, horizontal: Boolean) {
        if (!recordingActive) return
        if (isSensitive(node)) return
        val rect = boundsOf(node)
    ...
    /**
     * Stops recording and persists whatever was captured — single take,
     * no repeat demo required. If existingMacroId was set via start()
     * (Execute tab redo), this overwrites that macro's steps in place
     * (keeping its id/trigger/usage stats) instead of inserting a new row.
     */
    suspend fun stopAndSave(context: Context): RecordResult {
        recordingActive = false
        ...
        existingMacroId = null
        return RecordResult.Done(entity, duplicatesRemoved)
    }
