...
class JunAccessibilityService : AccessibilityService() {
    ...
    /**
     * IMPROVEMENT (multi-demo merge): a Volume press used to always mean
     * "recording is over, save it". Now it means "this ONE demo is over" —
     * RecordingEngine.stopDemo() decides whether that's enough (2 demos
     * agreed => Done), not enough yet (=> ask for another repeat of the
     * SAME task), or ambiguous (=> ask for a 3rd tie-breaker demo). Only
     * the terminal Done case disables recording mode and returns to the
     * app; every other case sends the user straight back to the home
     * screen to redo the task, staying in recording mode throughout.
     */
    private fun finishRecording() {
        val appContext = applicationContext
        CoroutineScope(Dispatchers.IO).launch {
            when (val outcome = RecordingEngine.stopDemo(appContext)) {
                is RecordingEngine.DemoOutcome.Empty -> {
                    ChatMessageInjector.postBotMessage(
                        appContext,
                        "Kuch record nahi hua is baar — koi valid step capture nahi hui (ho sakta hai sab sensitive fields the, ya kuch tap hi nahi hua). Wahi task dobara karo, khatam hone pe Volume dabao."
                    )
                    withContext(Dispatchers.Main) { pressHome() }
                }
                is RecordingEngine.DemoOutcome.NeedsMoreDemos -> {
                    ChatMessageInjector.postBotMessage(
                        appContext,
                        "Demo ${outcome.demoNumber}/${outcome.totalDemos} ho gaya (${outcome.stepsThisDemo} steps). " +
                            "Ab bilkul wahi task ek aur baar EXACTLY waise hi karo — isse main sirf un steps ko seekhungi jo dono baar consistent the, kisi bhi galti se hui extra ya missed tap ko chhod dungi. Khatam hone pe phir Volume Up/Down dabao."
                    )
                    RecordingEngine.beginNextDemo()
                    withContext(Dispatchers.Main) { pressHome() }
                }
                is RecordingEngine.DemoOutcome.NeedsTieBreaker -> {
                    ChatMessageInjector.postBotMessage(
                        appContext,
                        "Dono demo thodi alag nikli (is baar ${outcome.stepsThisDemo} steps) — confirm karne ke liye ek aur (3rd) baar wahi task karo. Jo kam se kam 2 baar match karega, wahi final macro mein rahega."
                    )
                    RecordingEngine.beginNextDemo()
                    withContext(Dispatchers.Main) { pressHome() }
                }
                is RecordingEngine.DemoOutcome.Done -> {
                    disableRecordingMode()
                    val note = if (outcome.discarded > 0)
                        " (${outcome.discarded} inconsistent step${if (outcome.discarded > 1) "s" else ""} hataye maine, jo sirf kuch demos mein hi aaye the — baaki sab demos mein match hue)"
                    else ""
                    val message = "Maine seekh liya! 🎉 \"${outcome.macro.displayPhrase}\" — ab agli baar ye bolne pe main khud kar dungi (${outcome.kept} steps$note)."
