...
class JunAccessibilityService : AccessibilityService() {
    ...
    private fun onPreviewSaved(appContext: Context) {
        CoroutineScope(Dispatchers.IO).launch {
            val message = when (val saveResult = RecordingEngine.confirmSave(appContext)) {
    ...
    suspend fun findBestMatchingNode(
        ...
        // BUGFIX (Phase 1a): recycle every candidate that ISN'T the winner.
        // This used to be skipped entirely (see collectAllNodes()'s old doc
        // comment) out of fear that two candidates could be the same
        // underlying node returned twice, and double-recycling one of them
        // would crash. That fear doesn't apply to `candidates` anymore:
        // addCandidate() above already guarantees every entry here is a
        // distinct logical node (duplicates were recycled and rejected
        // before ever reaching this set), so there's no double-recycle risk
        // left to protect against. Still wrapped in try/catch + log per
        // node, purely as a safety net to surface it on real devices if
        // this assumption ever turns out wrong.
        for (node in candidates) {
            if (node !== best) {
                try {
                    node.recycle()
                } catch (e: Exception) {
                    android.util.Log.w("JunAccessibility", "recycle() threw on a non-best candidate — investigate: ${e.message}")
                }
            }
        }
