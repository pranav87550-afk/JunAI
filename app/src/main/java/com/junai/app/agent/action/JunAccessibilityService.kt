...
class JunAccessibilityService : AccessibilityService() {
    ...
    fun detectBlockingOverlay(): String? {
        ...
        for (w in ownWindows) {
            when (w.type) {
                // BUGFIX: same class of false-positive as the TYPE_SYSTEM
                // fix below — an IME window can stay present in the
                // window list even when no keyboard is actually visible
                // on screen (e.g. right after coming off a lock-screen
                // PIN/pattern entry, before Android tears the window down).
                // The old check treated the mere EXISTENCE of a
                // TYPE_INPUT_METHOD window as "a keyboard is covering the
                // screen" — which is exactly what blocked a plain Quick
                // Settings tile tap with no keyboard anywhere in sight.
                // Same isActive() gate as TYPE_SYSTEM: only a keyboard
                // that's actually up and receiving input should count.
                // NOTE: same on-device-unverified caveat as below — please
                // confirm this specific scenario (lock screen → quick
                // settings) no longer false-blocks on your OPPO.
                android.view.accessibility.AccessibilityWindowInfo.TYPE_INPUT_METHOD -> {
                    if (w.isActive) return "keyboard"
                }
                // BUGFIX: TYPE_SYSTEM isn't exclusive to genuine blocking
                // popups (permission dialogs, the power menu, "screen
                // overlay detected" warnings) — it's ALSO the type Android
                // reports for the status bar and navigation bar, which are
                // present on essentially every screen and never actually
                // cover/intercept a tap on the app's own content. The old
                // check treated the mere EXISTENCE of any TYPE_SYSTEM window
                // as "something is blocking the screen", which meant a step
                // could be wrongly reported as blocked by a "system_popup"
                // that was never actually visible — exactly this kind of
                // false stop. isActive() is the documented way to tell
                // "just present, not receiving input" (status/nav bar) apart
                // from "actually has the user's attention right now" (a real
                // interruptive system dialog) — only the latter should count.
                // NOTE: not verified on-device yet (no Android runtime here)
                // — this is the correct API-level distinction, but please
                // confirm on your OPPO that isActive() actually behaves this
                // way for the status/nav bar on it specifically; OEM skins
                // occasionally report windows non-standardly.
                android.view.accessibility.AccessibilityWindowInfo.TYPE_SYSTEM -> {
                    if (w.isActive) return "system_popup"
                }
