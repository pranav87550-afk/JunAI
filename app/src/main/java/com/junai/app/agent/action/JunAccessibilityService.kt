...
class JunAccessibilityService : AccessibilityService() {
    ...
    private fun finishRecording() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val message = if (saved != null) {
                val cleanupNote = if (RecordingEngine.lastDuplicatesRemoved > 0)
                    " (${RecordingEngine.lastDuplicatesRemoved} duplicate step hataye maine, wo galti se do baar capture ho gaye the)"
                else ""
                "Maine seekh liya! 🎉 \"${saved.displayPhrase}\" — ab agli baar ye bolne pe main khud kar dungi (${saved.stepCount} steps)$cleanupNote."
            } else {
        ...
        if (RecordingEngine.isRecording) {
            ...
            if (RecordingEngine.isStale()) {
                ...
                AccessibilityEvent.TYPE_VIEW_CONTEXT_CLICKED -> {
                    event.source?.let { node ->
                        try { RecordingEngine.captureLongPress(node) } finally { node.recycle() }
                    }
                }
    ...
    fun findBestMatchingNode(
        ...
        if (!text.isNullOrBlank()) {
            root.findAccessibilityNodeInfosByText(text)?.let { candidates.addAll(it) }
        }
        ...
        for (node in candidates) {
            ...
            if (!text.isNullOrBlank() && nodeText != null) {
                score += if (nodeText.equals(text, ignoreCase = true)) 30.0
                else if (nodeText.contains(text, ignoreCase = true)) 15.0 else 0.0
            }

            val nodeDesc = node.contentDescription?.toString()
            if (!contentDescription.isNullOrBlank() && nodeDesc != null) {
                score += if (nodeDesc.equals(contentDescription, ignoreCase = true)) 20.0
                else if (nodeDesc.contains(contentDescription, ignoreCase = true)) 10.0 else 0.0
            }
