...
class JunAccessibilityService : AccessibilityService() {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        if (RecordingEngine.isRecording) {
            ...
            if (RecordingEngine.isStale()) {
            ...
            } else if (event.source?.rangeInfo != null) {
                // PHASE D (slider/SeekBar capture): checked before the
                // normal per-event-type dispatch below, since which
                // AccessibilityEvent type actually carries a slider's
                // progress change varies by implementation (some fire
                // TYPE_VIEW_SCROLLED, some TYPE_WINDOW_CONTENT_CHANGED,
                // Compose sliders can differ again) — RangeInfo's
                // presence on the source node is the one reliable,
                // implementation-independent signal that this is a
                // slider at all, so check for it directly rather than
                // trying to enumerate every event type that might carry it.
                val node = event.source
                if (node != null) {
                    try { RecordingEngine.captureSliderChange(node, applicationContext) } finally { node.recycle() }
                }
                ...
                AccessibilityEvent.TYPE_VIEW_SCROLLED -> {
                    val node = event.source
                    if (node == null) {
                        // BUGFIX: confirmed on-device (the diagnostic Toast
                        // below fired for exactly this — com.android.
                        // systemui QS page swipes) — was log+Toast+drop
                        // before, now falls back to the event's own fields,
                        // same idea as captureTapFromEventFallback for TAP.
                        // See captureScrollFromEventFallback's doc comment
                        // for how this becomes replayable without a node.
                        android.util.Log.w("JunAccessibility", "Recording: SWIPE event had null source — using event fallback. pkg=${event.packageName} class=${event.className}")
                        ...
                        RecordingEngine.captureScrollFromEventFallback(event, forward, horizontal, applicationContext)
                    } else {
                        try {
                            val horizontal: Boolean
                            val forward: Boolean
                ...
                // PHASE D (system-nav: Home) — see
                // RecordingEngine.captureHomePress()'s doc comment for why
                // only Home is captured this way and not Back/Recents too.
                // PHASE F extends this to Recents (see captureRecentsOpen's
                // doc comment for the className-pattern caveat).
                AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED -> {
                    val pkg = event.packageName?.toString()
                    val cls = event.className?.toString()
                    if (pkg != null && pkg == MacroReplayEngine.getDefaultLauncherPackage(applicationContext)) {
                        RecordingEngine.captureHomePress(applicationContext)
                    } else if (cls != null && (cls.contains("Recents", ignoreCase = true) || cls.contains("Overview", ignoreCase = true))) {
                        RecordingEngine.captureRecentsOpen(applicationContext)
                    }
                }
            }
        }
        ...
        // BUGFIX (root cause of "<home_screen>"/any-package foreground
        // checks intermittently failing right after a text field was
        // involved — confirmed on-device via log: ScreenContextEngine
        // logged currentApp="com.google.android.inputmethod.latin" at the
        // exact moment Gboard's own KeyboardWrapper/LatinIme logs show it
        // loading): the soft keyboard fires its own accessibility events
        // (packageName = the IME's package) as it shows/hides, same as any
        // app would. Since currentApp was overwritten by whichever event
        // arrived last with no filtering, a keyboard show/hide event
        // landing after the real navigation could clobber currentApp with
        // the IME's package — even though the actual foreground app/surface
        // hadn't changed at all. Skip IME events for currentApp tracking
        // entirely; every other use of these events (recording, snapshot)
        // is unaffected.
        val imePackage = imePackage()
        val eventPackage = event.packageName?.toString()
        eventPackage?.takeIf { it.isNotBlank() && it != imePackage }?.let { ScreenContextEngine.updateCurrentAppOnly(it) }
    ...
    private fun publishScreenSnapshot(root: AccessibilityNodeInfo, eventPackage: String? = null) {
        ...
        ScreenContextEngine.updateContext(
            currentApp = eventPackage?.takeIf { it.isNotBlank() && it != imePackage() } ?: root.packageName?.toString() ?: "unknown",
    ...
    // Cached (not re-queried per event — this is on the hot path of every
    // single accessibility event): the device's current default input
    // method's package name, e.g. "com.google.android.inputmethod.latin".
    // Re-resolved lazily on first use per service instance; if the user
    // switches keyboards mid-session this could go stale, but a wrong IME
    // package here only means we stop filtering an old keyboard's events
    // (or start filtering a new one late) — never worse than the
    // unfiltered behavior this replaces.
    private var cachedImePackage: String? = null
    private var imePackageResolved = false
    private fun imePackage(): String? {
        if (!imePackageResolved) {
            imePackageResolved = true
     ...
     */
    /**
     * BUGFIX (root cause of "Quick Settings panel hi nahi khulta" during
     * replay): checks the LIVE window list for a com.android.systemui
     * window right now — no dependency on any accessibility event having
     * fired. See isQuickSettingsWindowOpen()'s doc comment for why the
     * event-driven alternative (ScreenContextEngine.currentApp) is
     * specifically unreliable for this surface on this OEM.
     */
    fun isQuickSettingsWindowOpen(): Boolean {
        val ownWindows = try { windows } catch (e: Exception) { null } ?: return false
        return ownWindows.any { it.root?.packageName?.toString() == "com.android.systemui" }
    }

    /**
     * BUGFIX (root cause of "Quick Settings panel hi nahi khulta"):
     * performGlobalAction()'s return value only means the action was
     * dispatched to the OS — it says nothing about whether the panel
     * actually appeared. On this OEM (ColorOS), the global action
     * intermittently no-ops (most often when fired right after another
     * global action/gesture, a known OEM debounce quirk) — the old code
     * trusted the dispatch result, so replay would confidently move on to
     * tap a tile that was never actually on screen. Now polls the live
     * window list (isQuickSettingsWindowOpen(), not any event) for up to
     * ~1.2s after dispatching, and retries once if it never showed up.
     */
    suspend fun expandQuickSettings(): Boolean {
        if (isQuickSettingsWindowOpen()) return true // already open — don't re-fire (a second GLOBAL_ACTION_QUICK_SETTINGS can toggle it closed)
        repeat(2) { attempt ->
            val dispatched = if (android.os.Build.VERSION.SDK_INT >= 31) {
                performGlobalAction(GLOBAL_ACTION_QUICK_SETTINGS)
            } else {
                @Suppress("DEPRECATION")
                performGlobalAction(GLOBAL_ACTION_NOTIFICATIONS)
            }
            if (!dispatched && attempt == 1) return false
            val deadline = System.currentTimeMillis() + 1200L
            while (System.currentTimeMillis() < deadline) {
                if (isQuickSettingsWindowOpen()) return true
                delay(100L)
            }
            // Didn't open — loop retries once more before giving up.
        }
        return false
    }
