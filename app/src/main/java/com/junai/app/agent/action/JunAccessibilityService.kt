        ...
        if (RecordingEngine.isRecording) {
            ...
            if (RecordingEngine.isStale()) {
                ...
                AccessibilityEvent.TYPE_VIEW_SCROLLED -> {
                    val node = event.source
                    if (node == null) {
                        // DIAGNOSTIC (upgraded to a visible Toast — see
                        // reasoning below): this is the leading suspect for
                        // "QS page swipe / home-screen swipe sometimes
                        // doesn't get recorded" — some swipeable panels'
                        // gesture fires TYPE_VIEW_SCROLLED without a
                        // source node attached, especially on
                        // gesture-navigation devices where the system
                        // intercepts part of the gesture first.
                        ...
                        // BUGFIX (on-device visibility, same reasoning as
                        // the earlier TAP null-source Toast): Pranav has no
                        // logcat access (phone-only workflow). Confirmed
                        // exactly this pattern once already for QS tile
                        // taps — this is the same diagnostic technique,
                        // now applied to the swipe case specifically to
                        // confirm (not guess) before writing a real fix.
                        // Remove once confirmed, same as before.
                        android.widget.Toast.makeText(applicationContext, "Recording: swipe had no source (pkg=${event.packageName}) — step skipped", android.widget.Toast.LENGTH_SHORT).show()
                    } else {
                        try {
