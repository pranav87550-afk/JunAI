            ...
            if (RecordingEngine.isStale()) {
                ...
                AccessibilityEvent.TYPE_VIEW_CLICKED -> {
                    ...
                    if (node == null) {
                        ...
                        // BUGFIX (on-device visibility): Pranav has no way
                        // to check logcat (phone-only workflow, no local
                        // machine) — this diagnostic was previously
                        // invisible to him. A Toast makes it directly
                        // observable without any extra tooling, specifically
                        // to confirm/rule out whether this exact null-source
                        // case is the cause of "kuch record nahi hua" in
                        // Quick Settings.
                        android.widget.Toast.makeText(applicationContext, "Recording: tap had no source (pkg=${event.packageName}) — step skipped", android.widget.Toast.LENGTH_SHORT).show()
                    } else {
                ...
                AccessibilityEvent.TYPE_VIEW_CONTEXT_CLICKED -> {
                    ...
                    if (node == null) {
                        ...
                        android.widget.Toast.makeText(applicationContext, "Recording: long-press had no source (pkg=${event.packageName}) — step skipped", android.widget.Toast.LENGTH_SHORT).show()
                    } else {
                ...
                AccessibilityEvent.TYPE_VIEW_TEXT_CHANGED -> {
                    ...
                    if (node == null) {
                        ...
                        android.widget.Toast.makeText(applicationContext, "Recording: text-change had no source (pkg=${event.packageName}) — step skipped", android.widget.Toast.LENGTH_SHORT).show()
                    } else {
                        try {
