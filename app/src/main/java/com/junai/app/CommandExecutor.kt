...
object CommandExecutor {
    ...
    // Single managed scope for this singleton — SupervisorJob so one failure
    // doesn't cancel other in-flight searches
    ...
    fun execute(context: Context, text: String) {
        ...
        when (result.intent) {
            ...
            IntentDetector.Intent.SEND_MESSAGE -> {
                if (target.isNotEmpty()) sendMessage(context, target)
                else respond(context, "Kisko message karun? 💬")
            }
            ...
            IntentDetector.Intent.STOP_MUSIC -> {
                sendMusicAction(context, "STOP")
                respond(context, "Music band! ⏹️")
            }
    ...
    private fun sendMessage(context: Context, name: String) {
        val contactPermission = android.Manifest.permission.READ_CONTACTS

        if (ContextCompat.checkSelfPermission(context, contactPermission) != PackageManager.PERMISSION_GRANTED) {
            respond(context, "Contacts permission chahiye message ke liye!")
            return
        }
        ...
        if (number != null) {
            respond(context, "Opening WhatsApp for $name... 💬")
            try {
                val intent = Intent(Intent.ACTION_VIEW).apply {
                    data = android.net.Uri.parse("https://wa.me/${number!!.replace(" ", "").replace("-", "")}")
