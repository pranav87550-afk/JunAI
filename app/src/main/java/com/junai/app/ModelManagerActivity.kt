            ...
            ModelStateStore.observe(this, model.id).observe(this, Observer { info ->
                when (info.status) {
                    ...
                    ModelStateStore.Status.READY -> {
                        ...
                        progressBar.visibility = android.view.View.GONE
                        // Was permanently disabled here — no way to
                        // clear a corrupt-but-existing file (exactly
                        // what happened with EmbeddingGemma downloading
                        // truncated on a bad connection but still
                        // passing the old plain-existence check). Now
                        // tapping it offers to delete + redownload.
                        actionButton.text = "Downloaded ↻"
                        actionButton.isEnabled = true
                        actionButton.setOnClickListener {
                            androidx.appcompat.app.AlertDialog.Builder(this)
                                .setTitle("Re-download ${model.displayName}?")
                                .setMessage("Deletes the current file and downloads it again from scratch. Use this if the model is failing to load.")
                                .setPositiveButton("Re-download") { _, _ ->
                                    ModelDownloadManager.deleteModel(this, model.id)
