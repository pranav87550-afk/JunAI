...
class LearningCenterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tabKnowledge.setOnClickListener {
            activeTab = "KNOWLEDGE"
            showKnowledgeTab()
        }
        tabCommands.setOnClickListener {
            activeTab = "COMMANDS"
            showCommandsTab()
        }
    ...
    private fun showAnalyticsTab() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            withContext(Dispatchers.Main) {
                ...
                if (failures.isEmpty()) {
                    val noFail = TextView(this@LearningCenterActivity).apply {
                        text = "✅ No failures yet!"
                        setTextColor(android.graphics.Color.WHITE)
                        textSize = 13f
                        setPadding(0, 8, 0, 8)
                    }
                    inner.addView(noFail)
                } else {
            ...
            .setPositiveButton("Save") { _, _ ->
                ...
                if (answer.isNotEmpty()) {
                    CoroutineScope(Dispatchers.IO).launch {
                        ...
                        withContext(Dispatchers.Main) {
                            Toast.makeText(this@LearningCenterActivity, "Knowledge + Chain saved! ✅", Toast.LENGTH_SHORT).show()
                            showPendingTab()
                        }
                    }
                }
