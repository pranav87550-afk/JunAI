...
class LearningCenterActivity : AppCompatActivity() {
     ...
     * deleting it and losing its trigger phrase + usage stats. Same
     * recording flow as a fresh "sikhao", but RecordingEngine.start's
     * existingMacroId makes the freshly recorded steps overwrite this
     * row in place instead of inserting a new macro.
     */
    private fun showRedemonstrateDialog(item: com.junai.app.learning.RecordedMacroEntity) {
        ...
        AlertDialog.Builder(this, R.style.DarkDialog)
            ...
            .setPositiveButton("Start") { _, _ ->
                // BUGFIX (Phase 1c): don't start recording while a replay
                // or a live agent task is mid-flight — see
                // RecordingEngine.busyReasonBeforeStart()'s comment for
                // why. Needs a coroutine since the DB-backed check is
                // suspend; svc/UI calls stay on Main.
                CoroutineScope(Dispatchers.IO).launch {
                    val busyReason = com.junai.app.agent.action.RecordingEngine.busyReasonBeforeStart(this@LearningCenterActivity)
                    withContext(Dispatchers.Main) {
                        if (busyReason != null) {
                            Toast.makeText(this@LearningCenterActivity, busyReason, Toast.LENGTH_LONG).show()
                            return@withContext
                        }
                        ...
                        Toast.makeText(this@LearningCenterActivity, "Recording shuru! Volume button dabao jab ho jaaye.", Toast.LENGTH_LONG).show()
                        svc.pressHome()
                        finish()
                    }
                }
            ...
            withContext(Dispatchers.Main) {
                ...
                recyclerView.adapter = object : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
                    ...
                    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
                        ...
                        val categoryView = holder.itemView.findViewById<TextView>(R.id.knowledgeCategory)
                        // IMPROVEMENT (Phase 1g): items flagged via thumbs-down
                        // (needsCorrection = true) now show up here too, since
                        // Negative Responses and this tab share the same rows —
                        // this badge makes it clear the visible answer is
                        // stale/known-bad and awaiting a fix in Negative
                        // Responses, rather than looking like a confirmed,
                        // correct trained answer.
                        if (item.needsCorrection) {
                            categoryView.text = "⚠️ Needs correction • 📁 ${item.category} • Asked: ${item.timesAsked}x"
                            categoryView.setTextColor(android.graphics.Color.parseColor("#E53935"))
                        } else {
                            categoryView.text = "📁 ${item.category} • Asked: ${item.timesAsked}x"
                            categoryView.setTextColor(android.graphics.Color.parseColor("#888888"))
                        }
    ...
    private fun showExecuteConfirmDialog(item: LearningItem) {
        ...
        AlertDialog.Builder(this, R.style.DarkDialog)
            ...
            .setMessage(
                ...
                "3. Password/PIN fields kabhi record nahi hongi\n" +
                "4. Khatam hone pe Volume Up ya Down daba do\n\n" +
                "Ready?"
            )
            .setPositiveButton("Start") { _, _ ->
                // BUGFIX (Phase 1c): same busy check as showRedemonstrateDialog above.
                CoroutineScope(Dispatchers.IO).launch {
                    val busyReason = com.junai.app.agent.action.RecordingEngine.busyReasonBeforeStart(this@LearningCenterActivity)
                    withContext(Dispatchers.Main) {
                        if (busyReason != null) {
                        ...
                        com.junai.app.agent.action.RecordingEngine.start(
                            ...
                            sourcePendingItemId = item.id
                        )
                        svc.enableRecordingMode()
                        // BUGFIX: this used to flip the item to
                        // TRAINED_EXECUTE right here, the instant recording
                        // STARTED — not when it was actually saved. Once
                        // Phase 2 added Discard, that meant discarding a
                        // recording still permanently removed the item
                        // from Pending (status already flipped), AND the
                        // phrase could never re-enter Pending later either
                        // (logFailure()'s dedup check is status-agnostic —
                        // it just checks a row exists). The status update
                        // now happens in JunAccessibilityService.onPreviewSaved()
                        // instead, only after the user actually taps Save.
                        Toast.makeText(this@LearningCenterActivity, "Recording shuru! Volume button dabao jab ho jaaye.", Toast.LENGTH_LONG).show()
                        svc.pressHome()
                        finish()
                    }
