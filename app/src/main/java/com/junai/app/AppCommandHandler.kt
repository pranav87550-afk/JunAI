...
class AppCommandHandler(
    ...
    // IMPROVEMENT (Phase 1f): these three used to return Unit — every
    // caller (ChatIntentHandler, TrainedCommandHandler) had no way to know
    // whether the request was actually fulfilled (app opened / call
    // placed / message flow started) or fell back to a "couldn't do it"
    // outcome (Play Store search shown instead, contact not found,
    // permission not yet granted) — so a repeatedly-failing "open X" or
    // "call Y" phrase could never be logged as a failure and could never
    // reach the Pending tab for Pranav to teach a fix. Returning a Boolean
    // lets the caller decide to log it. Note: the permission-not-granted
    // branches also return false here — technically a different kind of
    // "didn't do it" (device permission, not something teaching fixes),
    // but harmless either way if it shows up in Pending; flagging it here
    // rather than silently picking a stance on it.
    ...
    fun makeCall(name: String): Boolean {
    ...
    fun sendMessage(contactName: String): Boolean {
        val contactPerm = Manifest.permission.READ_CONTACTS
    ...
    private fun reply(text: String) {
        chatAdapter.addMessage(ChatMessage(text, isUser = false, isSystem = true))
        onSaveChat()
    }
}
