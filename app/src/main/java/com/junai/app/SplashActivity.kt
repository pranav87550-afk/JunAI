...
class SplashActivity : AppCompatActivity() {
    ...
    /**
     * If JunApplication's global exception handler caught a crash last
     * run, its stack trace is sitting in filesDir/last_crash.txt — show
     * it here so it's visible (and screenshot-able) without needing a
     * laptop/logcat. Deletes the file after showing so the same crash
     * doesn't pop up again on every subsequent launch.
     */
    private fun showPendingCrashLogIfAny() {
        val file = java.io.File(filesDir, JunApplication.CRASH_LOG_FILENAME)
        if (!file.exists()) return
        val log = try { file.readText() } catch (e: Exception) { return }
        file.delete()
        ...
        androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("Jun crashed last time — here's why")
            .setView(scrollView)
