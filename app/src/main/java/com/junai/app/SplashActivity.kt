...
class SplashActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Posted rather than called directly — showing a dialog before
        // the window is fully attached to the WindowManager can throw
        // BadTokenException on some Android versions/OEM skins. post()
        // runs this right after the current layout pass instead of
        // synchronously inside onCreate(), which is enough to avoid that.
    ...
    /**
     * Fix: Sound aur swipe animation ek saath *feel* hone chahiye.
     *
     * Pehle: play() → startActivity() → finish() immediately
     * Sound stream Activity destroy hone se cut off ho jaata tha.
     *
     * Phir: play() aur startActivity() ek hi line mein back-to-back call
     * kiye — lekin SoundPool.play() ka latency ~0ms hai jabki naye
     * MainActivity ka window banna + layout inflate + first draw hone mein
     * real delay lagta hai (is phone — SM6115 — pe zyada noticeable).
     * Isliye sound pehle sunayi deta tha, swipe thoda baad mein dikhta tha.
     *
     * Ab: startActivity()/transition pehle trigger karo, sound ko chhoti si
     * SOUND_SYNC_DELAY_MS delay se play karo taaki dono roughly saath mehsoos
     * ho. Agar on-device test karke lage sound ab bhi thoda aage/peeche hai,
     * to bas neeche wali delay value tweak kar dena (50ms steps mein try karo).
     */
    private val SOUND_SYNC_DELAY_MS = 120L
    ...
    private fun goToMainActivity() {
        ...
        // Sound thodi delay se — naye activity window ke actually visible
        // hone tak wait karta hai, taaki audio aur swipe saath lagein
        if (soundLoaded) {
            handler.postDelayed({
        ...
        handler.postDelayed({ finish() }, SOUND_SYNC_DELAY_MS + 400)
    }
    ...
    private fun showPendingCrashLogIfAny() {
        try {
            ...
            // This is a debugging aid layered on top of the real app —
            // it must never be the thing that crashes the app. If
            // showing the dialog itself fails for any reason (theme,
            // window-token timing, whatever), just skip it silently
            // rather than taking the whole launch down with it.
            android.util.Log.e("SplashActivity", "showPendingCrashLogIfAny() itself failed: ${e.message}", e)
        }
    }
