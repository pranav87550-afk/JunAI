...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private var networkDotAnimator: AnimatorSet? = null
...
findViewById<LinearLayout>(R.id.menuVoiceCommands).setOnClickListener {
        ...
        sendButton.setOnClickListener {
            ...
            when (intentResult.intent) {
                ...
                IntentDetector.Intent.SEARCH_WEB -> {
                    ...
                    if (query.isNotEmpty()) searchAndRespond(query, recyclerView) else enableSendButton()
                }
    ...
    // Register network callback for fast detection
    ...
    private fun updateNetworkStatus(isOnline: Boolean) {
        val dotDrawable = networkDot.background as? GradientDrawable
        if (isOnline) {
            dotDrawable?.setColor(0xFF43A047.toInt())
            networkStatusText.text = "Online"
            networkStatusText.setTextColor(0xFF43A047.toInt())
        } else {
            dotDrawable?.setColor(0xFFE53935.toInt())
            networkStatusText.text = "Offline"
            networkStatusText.setTextColor(0xFFE53935.toInt())
        }
        // Pulse animation
        networkDotAnimator?.cancel()
        val scaleX = ObjectAnimator.ofFloat(networkDot, "scaleX", 1f, 1.5f, 1f)
        val scaleY = ObjectAnimator.ofFloat(networkDot, "scaleY", 1f, 1.5f, 1f)
        scaleX.duration = 1000
        scaleY.duration = 1000
        scaleX.repeatCount = ObjectAnimator.INFINITE
        scaleY.repeatCount = ObjectAnimator.INFINITE
        networkDotAnimator = AnimatorSet().also {
