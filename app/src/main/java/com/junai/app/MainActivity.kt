...
class MainActivity : AppCompatActivity() {
    ...
    // --- Network status ---
    private lateinit var networkDot: TextView
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (storedAnswer != null) {
                withContext(Dispatchers.Main) {
                    ...
                    recyclerView.scrollToPosition(messages.size - 1)
                    saveChat()
                    if (voiceEnabled && ttsHelper.isReady) ttsHelper.speak(storedAnswer)
                }
                return@launch
            }
    ...
    private fun storeAndRespond(
        ...
        recyclerView.scrollToPosition(messages.size - 1)
        saveChat()
        if (voiceEnabled && ttsHelper.isReady) ttsHelper.speak(answer)
    }
    ...
    private fun updateNetworkStatus(isOnline: Boolean) {
        ...
        networkDotAnimator?.cancel()
        networkDotAnimator = ObjectAnimator.ofFloat(networkDot, "alpha", 1f, 0.2f, 1f).apply {
            duration = 1000
