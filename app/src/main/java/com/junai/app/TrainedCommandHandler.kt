...
class TrainedCommandHandler(
    ...
    fun handle(intent: String, target: String, text: String, recyclerView: RecyclerView): Boolean {
        val handled = when (intent) {
            ...
            "TELL_BATTERY" -> {
                ...
                reply("Command samajh nahi aaya! 🤔", isSystem = true)
                false
            }
        }
    ...
    private fun reply(text: String, isSystem: Boolean = false) {
        chatAdapter.addMessage(ChatMessage(text, isUser = false, isSystem = isSystem))
    }
}
